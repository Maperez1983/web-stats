...
def build_weekly_staff_brief(
    *,
    ...
    available_cards = [card for card in cards if _safe_int(card.get("player_id")) not in injury_ids | sanction_ids]
    ...
    starter_names = [_clean_text(player.get("name"), "Jugador") for player in probable_eleven[:5]]
    ...
    return {
        ...
        "headline": (f"{available_count} disponibles de {total_players} " f"para {match_info['opponent']}"),
        "alerts": alerts,
