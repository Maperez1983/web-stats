...
def _stripe_sync_workspace_from_subscription(workspace, subscription, *, price_id: str = "") -> None:
    ...
    try:
        ...
        logger.debug(
            "No se pudieron calcular entitlements Stripe para workspace %s",
...
def stripe_webhook(request):
    ...
    # Idempotencia.
    try:
        obj, created = StripeEventLog.objects.get_or_create(
            event_id=event_id, defaults={"event_type": event_type, "payload": {}}
        )
        if not created:
    ...
    try:
        ...
        if event_type == "checkout.session.completed":
            ...
            if workspace and customer:
                ...
                workspace.plan_key = workspace.plan_key or str(
                    getattr(session, "metadata", {}).get("plan_key") or "pro"
                )
            ...
            if workspace and subscription_id:
                try:
                    ...
                    try:
                        price_id = (
                            str(sub.items.data[0].price.id) if getattr(sub, "items", None) and sub.items.data else ""
                        )
                    except Exception:
                        logger.debug(
                            "No se pudo leer price_id de subscription Stripe %s", subscription_id, exc_info=True
                        )
                        price_id = ""
                    _stripe_sync_workspace_from_subscription(workspace, sub, price_id=price_id)
                except Exception:
                    logger.debug(
                        "No se pudo recuperar subscription Stripe %s tras checkout", subscription_id, exc_info=True
                    )
            ok = True
            ...
            if workspace:
                ...
                workspace.save(
                    update_fields=["stripe_customer_id", "stripe_subscription_id", "subscription_status", "updated_at"]
                )
            ok = True
...
def _workspace_from_request_for_billing(request):
    ...
    if not workspace_context.can_manage_workspace(
...
def billing_page(request):
    ...
    manual_msg = ""
    manual_err = ""
    allow_manual = bool(getattr(settings, "DEBUG", False)) or str(
        os.getenv("ALLOW_MANUAL_BILLING", "0") or ""
    ...
    try:
        for key in ("trainer", "match", "analysis"):
            addons_available[key] = bool(
                (price_map.get((key, "month")) or "").strip() or (price_map.get((key, "year")) or "").strip()
            )
    except Exception:
        addons_available = {}
    return render(
...
def apple_receipt_api(request):
    ...
    try:
        ...
        return api_error(
            f"No se pudo validar con Apple: {exc.__class__.__name__}", status=502, code="apple_verify_failed"
        )
    ...
    if not entitlements:
        return api_error(
            "No hay suscripciones Apple activas para este club.", status=402, code="apple_no_active_products"
        )
    ...
    return api_ok(
        {"subscription_status": "active", "paid_modules": entitlements, "product_ids": sorted(active_product_ids)}
    )
...
def billing_checkout_session_api(request):
    ...
    selected_modules = [
        (
            "trainer"
            if m in {"entrenador", "core", "starter", "basic"}
            else "match" if m == "partido" else "analysis" if m == "analisis" else m
        )
