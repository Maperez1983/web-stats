...
def workspace_preference_get_api(request):
    ...
    if not workspace_context.can_view_workspace(request.user, workspace) and not workspace_context.can_access_platform(
        request.user
    ):
        ...
        return JsonResponse(
            {
                ...
                "last_sync_at": (
                    getattr(context, "last_sync_at", None).isoformat() if getattr(context, "last_sync_at", None) else ""
                ),
            }
        )
