...
class PlatformWorkspaceTeamDetailTests(TestCase):
    def test_team_detail_hides_google_urls_from_stadium_fields(self):
        ...
        response = self.client.get(
            reverse("platform-workspace-team-detail", args=[workspace.id, cadete.id]), secure=True
        )
...
class DashboardCompetitionSeasonalityTests(TestCase):
    ...
    def test_shared_group_before_september_does_not_show_setup_error(self, mock_localdate):
        ...
        group = Group.objects.create(
            season=season, name="Grupo pendiente", slug="grupo-pendiente", external_id="shared-group"
        )
        primary_team = Team.objects.create(
            name="Benagalbón", slug="home-preseason-primary", short_name="BEN", group=group, is_primary=True
        )
        cadete = Team.objects.create(
            name="Benagalbon c. d.", slug="home-preseason-cadete", short_name="CAD", category="CADETE", group=group
        )
        benjamin = Team.objects.create(
            name="Benjamín", slug="home-preseason-benjamin", short_name="BENJ", category="Benjamín", group=group
        )
        workspace = Workspace.objects.create(
            name="Benagalbón",
        ...
        response = self.client.get(
            reverse("dashboard-data"), {"workspace": workspace.id, "team": cadete.id, "home": "club"}, secure=True
        )
...
class StaffAccessInvitationTests(TestCase):
    def _create_workspace(self, username="staff-owner"):
        ...
        team = Team.objects.create(
            name=f"Staff Team {username}", slug=f"staff-team-{username}", short_name="STF", is_primary=True
        )
        workspace = Workspace.objects.create(
    ...
    def test_staff_invitation_does_not_require_email(self):
        ...
        self.assertTrue(
            WorkspaceMembership.objects.filter(
                workspace=workspace, user=user, role=WorkspaceMembership.ROLE_MEMBER
            ).exists()
        )
        ...
        self.assertTrue(
            UserInvitation.objects.filter(user=user, is_active=True, accepted_at__isnull=True, email="").exists()
        )
    ...
    def test_platform_workspace_detail_staff_chips_link_to_staff_profile_with_photo(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertContains(
            response, f'class="team-staff-card" href="{reverse("staff-member-detail", args=[member.id])}"'
        )
        self.assertContains(response, reverse("staff-member-photo-file", args=[member.id]))
