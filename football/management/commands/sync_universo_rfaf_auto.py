...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        storage_default = os.getenv("RFAF_STORAGE_STATE_PATH", "").strip() or str(
            Path("data") / "input" / "rfaf_storage_state.json"
        )
        capture_default = os.getenv("UNIVERSO_CAPTURE_PATH", "").strip() or str(
            Path("data") / "input" / "universo-rfaf-capture.json"
        )
        snapshot_default = os.getenv("UNIVERSO_SNAPSHOT_PATH", "").strip() or str(
            Path("data") / "input" / "universo-rfaf-snapshot.json"
        )
        parser.add_argument(
            "--team-url",
