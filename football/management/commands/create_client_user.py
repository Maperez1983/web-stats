...
def _pick_workspace(slug: Optional[str]) -> Optional[Workspace]:
    ...
    active_club_workspaces = list(
        Workspace.objects.filter(kind=Workspace.KIND_CLUB, is_active=True).order_by("name", "id")
    )
...
def _pick_default_team_for_workspace(workspace: Workspace):
    ...
    default_link = WorkspaceTeam.objects.select_related("team").filter(workspace=workspace, is_default=True).first()
    if default_link and getattr(default_link, "team", None):
        return default_link.team
    first_link = WorkspaceTeam.objects.select_related("team").filter(workspace=workspace).order_by("id").first()
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--username", default="", help="Username (si se omite, se deriva del email o se autogenera)."
        )
        parser.add_argument(
            "--password",
            default="",
    ...
    def handle(self, *args, **options):
        ...
        if app_role not in valid_app_roles:
            self.stdout.write(
                self.style.WARNING(f'Rol de app inválido "{app_role}", usando "{AppUserRole.ROLE_GUEST}".')
            )
        ...
        if workspace_role not in valid_workspace_roles:
            self.stdout.write(
                self.style.WARNING(
                    f'Rol de workspace inválido "{workspace_role}", usando "{WorkspaceMembership.ROLE_VIEWER}".'
                )
            )
        ...
        if workspace:
            self.stdout.write(
                self.style.SUCCESS(f"Workspace: {workspace.slug} · rol={workspace_role} · app_role={app_role}")
            )
        else:
