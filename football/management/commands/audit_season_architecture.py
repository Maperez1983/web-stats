...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--json", action="store_true", dest="as_json", help="Devuelve la auditoría completa en JSON."
        )
        parser.add_argument(
            "--backfill", action="store_true", help="Asigna club_season a registros inferibles por fecha."
        )
    ...
    def handle(self, *args, **options):
        ...
        for workspace, report in zip(workspaces, reports):
            ...
            if options.get("backfill"):
                ...
                continue
            self.stdout.write(
                f"  temporadas: {report.get('season_count', 0)} · activa: {report.get('active_season_id') or '-'}"
            )
            for model_key, model_report in sorted((report.get("models") or {}).items()):
                self.stdout.write(
                    "  "
