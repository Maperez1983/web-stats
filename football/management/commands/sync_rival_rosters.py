...
from football.models import Competition, Group, Season, Team, TeamRosterSnapshot, TeamStanding
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if home_team_id:
            primary_team = (
                Team.objects.filter(id=home_team_id)
                .select_related("group", "group__season", "group__season__competition")
                .first()
            )
        if not primary_team:
            primary_team = (
                Team.objects.filter(is_primary=True)
                .select_related("group", "group__season", "group__season__competition")
                .first()
            )
        ...
        def _ensure_group_season():
            ...
            season_name = (
                f"{timezone.localdate().year}/{timezone.localdate().year + 1}"
                if timezone.localdate().month >= 7
                else f"{timezone.localdate().year - 1}/{timezone.localdate().year}"
            )
        ...
        if provider == TeamRosterSnapshot.PROVIDER_UNIVERSO:
            ...
            if not group_id:
                raise CommandError("Falta group-id para Universo RFAF. Pasa --group-id o configura Group.external_id.")
            ...
            def _roster_stats_rows(items):
                ...
                for row in items[:120]:
                    ...
                    try:
                        if (
                            int(row.get("pj") or 0) > 0
                            or int(row.get("minutes") or 0) > 0
                            or int(row.get("goals") or 0) > 0
                        ):
                            total += 1
                    except Exception:
                        continue
            ...
            prefer_preferente = str(os.getenv("RIVAL_ROSTER_PREFER_PREFERENTE", "true") or "").strip().lower() in {
                "1",
                "true",
                "yes",
                "on",
            ...
            if update_standings:
                ...
                if season and group_for_standings:
                    ...
                    for idx, row in enumerate(rows, start=1):
                        ...
                        ga = _safe_int(
                            row.get("goles_en_contra") or row.get("goals_against") or row.get("gc"), default=0
                        )
                        ...
                        TeamStanding.objects.update_or_create(
                            ...
                            defaults={
                                ...
                                "played": _safe_int(
                                    row.get("jugados") or row.get("played") or row.get("pj"), default=0
                                ),
                                "wins": _safe_int(row.get("ganados") or row.get("wins") or row.get("pg"), default=0),
                                "draws": _safe_int(
                                    row.get("empatados") or row.get("draws") or row.get("pe"), default=0
                                ),
                                "losses": _safe_int(
                                    row.get("perdidos") or row.get("losses") or row.get("pp"), default=0
                                ),
                                "goals_for": gf,
                                "goals_against": ga,
                                "goal_difference": gd,
                                "points": _safe_int(
                                    row.get("puntos") or row.get("points") or row.get("pt") or row.get("pts"), default=0
                                ),
                                "last_updated": timezone.now(),
                            },
                        )
                    if updated_team_ids:
                        TeamStanding.objects.filter(group=group_for_standings).exclude(
                            team_id__in=updated_team_ids
                        ).delete()
