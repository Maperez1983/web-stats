...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if team_id:
            team = (
                Team.objects.select_related("group", "group__season", "group__season__competition")
                .filter(id=team_id)
                .first()
            )
            if not team:
        ...
        season = seasons[0]
        season_id = str(
            season.get("id") or season.get("codigo") or season.get("cod_temporada") or season.get("season_id") or ""
        ).strip()
        ...
        for delegation in delegations:
            delegation_id = str(
                delegation.get("id")
                or delegation.get("codigo")
                or delegation.get("cod_delegacion")
            ...
            if normalized_competition_filter:
                competitions = [
                    ...
                    if normalized_competition_filter
                    in _norm(c.get("nombre") or c.get("competicion") or c.get("name") or "")
                ]
            if not competitions:
                continue
            ...
            for comp in competitions:
                comp_id = str(
                    comp.get("id")
                    or comp.get("codigo")
                    or comp.get("cod_competicion")
                    or comp.get("competition_id")
                    or ""
                ).strip()
                if not comp_id:
                    continue
                comp_label = (
                    str(comp.get("nombre") or comp.get("competicion") or comp.get("name") or "").strip() or comp_id
                )
                ...
                for group in groups:
                    group_id = str(
                        group.get("id") or group.get("codigo") or group.get("cod_grupo") or group.get("group_id") or ""
                    ).strip()
                    ...
                    if candidates_seen > limit_groups:
                        raise CommandError(
                            f"Se alcanzó el límite de grupos ({limit_groups}). Ajusta filtros o sube --limit-groups."
                        )
                    ...
                    if found:
                        group_label = (
                            str(group.get("nombre") or group.get("grupo") or group.get("name") or "").strip()
                            or f"group {group_id}"
                        )
                        matches.append(
                            {
                        ...
                        if not list_matches:
                            ...
                            self.stdout.write("")
                            self.stdout.write(
                                "Sugerencia: guarda este id en `Group.external_id` para ese equipo/categoría."
                            )
                            return
        ...
        if matches:
            ...
            return
        raise CommandError(
            f"No se encontró group-id (probados {tested}, candidatos {candidates_seen}) en {elapsed:.1f}s. Ajusta filtros."
        )
