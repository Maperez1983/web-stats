    ...
    with transaction.atomic():
        ...
        if config.with_group:
            group = Group.objects.create(
                season=season, name=f"Grupo {config.key} {run_id}", slug=f"grupo-{config.key}-{run_id}"
            )
        ...
        if config.include_sanction and players:
            ...
            players[-1].save(
                update_fields=["manual_sanction_active", "manual_sanction_reason", "manual_sanction_until"]
            )
        ...
        if render_html:
            ...
            dash_req = _make_request(
                admin_user, path=reverse("player-dashboard"), session_data=session_data, query_string="?refresh=1"
            )
            dash_resp = player_dashboard_page(dash_req)
            _write_bytes(scenario_dir / "player_dashboard.html", dash_resp.content)
            for player in players:
                detail_req = _make_request(
                    admin_user,
                    path=reverse("player-detail", args=[player.id]),
        ...
        audit_req = _request_json(
            admin_user, path=reverse("kpi-audit"), session_data=session_data, query=f"?team_id={club.id}&refresh=1"
        )
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "--matrix", action="store_true", help="Ejecuta una matriz de escenarios (cobertura amplia de variables)."
        )
        parser.add_argument(
            "--fuzz", type=int, default=0, help="Ejecuta N escenarios aleatorios (sin HTML por defecto)."
        )
        parser.add_argument("--seed", type=int, default=0, help="Seed para --fuzz (0 = aleatorio).")
        parser.add_argument(
            "--teams", type=int, default=15, help="Número de equipos en el grupo (afecta jornadas totales)."
        )
        ...
        parser.add_argument(
            "--only", type=str, default="", help="Ejecuta solo escenarios cuyo key contenga este texto."
        )
        parser.add_argument(
            "--render-html", action="store_true", help="Renderiza HTML incluso en modos --fuzz (más lento)."
        )
        parser.add_argument(
            "--persist-db", action="store_true", help="No hace rollback de la BD al finalizar cada escenario."
        )
        parser.add_argument(
            "--fail-on-issues", action="store_true", help="Falla (exit!=0) si kpi_audit detecta incoherencias."
        )
    ...
    def handle(self, *args, **options):
        ...
        if bool(options.get("matrix")):
            ...
            scenarios = [
                ScenarioConfig(
                    key="f11_baseline", label="F11 · baseline (registro-acciones)", with_group=True, game_format="f11"
                ),
                ScenarioConfig(
                    key="f7_baseline", label="F7 · prebenjamín (50')", with_group=True, game_format="f7", teams_count=10
                ),
                ScenarioConfig(
                    key="substitutions",
                    label="Sustituciones (entrada/salida)",
                    with_group=True,
                    game_format="f11",
                    substitutions=True,
                    matches_count=2,
                ...
                ScenarioConfig(
                    ...
                    label="Dedupe eventos duplicados",
                    with_group=True,
                    game_format="f11",
                ...
                ScenarioConfig(
                    key="mixed_sources",
                    label="Fuentes mixtas (registro vs excel)",
                    with_group=True,
                    game_format="f11",
                    mixed_sources=True,
                    matches_count=2,
                ...
                ScenarioConfig(
                    key="missing_zone",
                    label="Inferencia de zona (zona vacía)",
                    with_group=True,
                    game_format="f11",
                    missing_zone_inference=True,
                    matches_count=2,
                ),
                ScenarioConfig(
                    key="manual_lock_assists",
                    label="Manual-base bloquea asistencias",
                    with_group=True,
                    game_format="f11",
                    manual_lock_assists=True,
                    matches_count=1,
                ),
                ScenarioConfig(
                    key="manual_lock_totals",
                    label="Manual-base bloquea PJ/PT/min/goles",
                    with_group=True,
                    game_format="f11",
                    manual_lock_totals=True,
                    matches_count=1,
                ),
                ScenarioConfig(
                    key="no_group_isolation",
                    label="Sin group: aislamiento por nombre",
                    with_group=False,
                    game_format="f11",
                    isolate_no_group_name_collision=True,
                    matches_count=2,
                ),
                ScenarioConfig(
                    key="extra_time",
                    label="Fin partido con añadido (95')",
                    with_group=True,
                    game_format="f11",
                    end_minute_override=95,
                    matches_count=1,
                ),
                ScenarioConfig(
                    key="no_end_marker",
                    label="Sin FIN: inferencia de fin de partido",
                    with_group=True,
                    game_format="f11",
                ...
                ScenarioConfig(
                    key="starter_no_events",
                    label="Titular sin acciones: PJ/PT por alineación",
                    with_group=True,
                    game_format="f11",
                    starter_without_events=True,
                    matches_count=1,
                ),
                ScenarioConfig(
                    key="absences_rotation",
                    label="Ausencias rotativas: convocados vs roster",
                    with_group=True,
                    game_format="f11",
                    rotate_absences=True,
                    matches_count=3,
                ),
                ScenarioConfig(
                    key="cards_injury_sanction",
                    label="Tarjetas + lesión + sanción",
                    with_group=True,
                    game_format="f11",
                    include_cards=True,
                    include_injury=True,
                    include_sanction=True,
                    matches_count=2,
                ),
                ScenarioConfig(
                    key="live_events",
                    label="Acciones en vivo (touch-field) + final",
                    with_group=True,
                    game_format="f11",
                    live_events=True,
                    matches_count=1,
            ...
            for scenario in scenarios:
                ...
                result = _run_scenario(
                    config=scenario, base_output_dir=matrix_dir, render_html=True, persist_db=persist_db
                )
            ...
            _write_text(
                ...
                json.dumps(
                    {"ok": True, "issues_total": issues_total, "results": results}, ensure_ascii=False, indent=2
                ),
            )
            ...
            for item in results:
                ...
                dash_link = (
                    f"{rel}/player_dashboard.html" if (Path(item["dir"]) / "player_dashboard.html").exists() else ""
                )
                if dash_link:
                    lines.append(
                        f'<li><strong>{label}</strong> · <a href="{dash_link}">dashboard</a> · <a href="{rel}/kpi_audit.json">audit</a></li>'
                    )
                else:
        ...
        if fuzz_count > 0:
            ...
            for idx in range(max(1, fuzz_count)):
                ...
                cfg = ScenarioConfig(
                    ...
                    end_minute_override=(
                        95
                        if game_format == "f11" and rng.random() < 0.15
                        else (55 if game_format == "f7" and rng.random() < 0.15 else None)
                    ),
                ...
                result = _run_scenario(
                    config=cfg, base_output_dir=fuzz_dir, render_html=render_html, persist_db=persist_db
                )
                results.append(result)
                summary = result.get("audit_summary") or {}
                issues_total += int(summary.get("issues_total") or 0)
            _write_text(
                ...
                json.dumps(
                    {"ok": True, "seed": base_seed, "issues_total": issues_total, "results": results},
                    ensure_ascii=False,
                    indent=2,
                ),
            )
