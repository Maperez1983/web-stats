...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--login-url", default=os.getenv("RFAF_UNIVERSO_LOGIN_URL", "https://www.universorfaf.es/login")
        )
        parser.add_argument(
            "--target-url", default=os.getenv("RFAF_UNIVERSO_TARGET_URL", "https://www.universorfaf.es/dashboard")
        )
            ...
            try:
                ...
                def _on_response(response):
                    try:
                        ...
                        same_origin = (
                            parsed_url.scheme == parsed_target.scheme and parsed_url.netloc == parsed_target.netloc
                        )
                        if not same_origin:
                            return
                ...
                login_markers = "iniciar sesión" in page_text or "iniciar sesion" in page_text or "acceder" in page_text
                ...
                console_summary = " | ".join(f"{entry['type']}: {entry['text']}" for entry in console_events[-8:])
                ...
                raise CommandError(
                    ...
                    + (f"Submit disabled: {submit_disabled_before}. " if submit_disabled_before is not None else "")
