...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            self.stdout.write(
                f'Content-Type: {resp.headers.get("content-type", "") if getattr(resp, "headers", None) else ""}'
            )
            html = resp.text or ""
        ...
        if preferente_id and str(preferente_id).isdigit():
            try:
                ...
                self.stdout.write(
                    self.style.ERROR(f"_fetch_preferente_team_roster_via_json error: {type(exc).__name__}: {exc}")
                )
