...
from football.models import Competition, DataSource, Group, Season, Team, TeamStanding
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        header_cells = [cell.get_text(strip=True) for cell in standings_table.find("tr").find_all(["th", "td"])]
        ...
        for row in standings_table.find_all("tr")[1:]:
            ...
            standing_values = {
                ...
                "goals_against": parse_number(self.get_value(row_data, ("gc", "golscontra", "contra"))),
            ...
            TeamStanding.objects.update_or_create(
                ...
                team=team,
                defaults={
                    **{k: v for k, v in standing_values.items() if v is not None},
        ...
        self.stdout.write(
            self.style.SUCCESS(f"Actualizada clasificación ({len(updated)} equipos) para {group_name} {season_name}")
        )
    ...
    def find_standings_table(soup: BeautifulSoup) -> Optional[Any]:
        ...
        for table in soup.find_all("table"):
            ...
            if not header:
                continue
            header_texts = " ".join(cell.get_text(strip=True).lower() for cell in header.find_all(["th", "td"]))
