    ...
    for inp in inputs:
        ...
        if kind == "password":
            ...
            if input_type in ("text", "email") and any(
                token in low_name for token in ("user", "mail", "login", "usuario")
            ):
                return name
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if not user_field or not pass_field:
            raise CommandError(f"No se identificaron campos de credenciales (user={user_field}, pass={pass_field}).")
        ...
        self.stdout.write(f"Enviando credenciales a {post_url} (campos detectados: {user_field}/{pass_field})")
        try:
        ...
        if target_url:
            ...
            try:
                ...
                self.stdout.write(f"Respuesta target: {target_resp.status_code} · URL final: {target_resp.url}")
