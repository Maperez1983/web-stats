...
def _task_backup_payload(task, *, kind: str, reason: str, actor_username: str = "") -> dict:
    ...
    return {
        ...
        "task": {
            ...
            "deleted_at": (
                (
                    timezone.localtime(getattr(task, "deleted_at")) if getattr(task, "deleted_at", None) else None
                ).isoformat()
            ...
            "updated_at": (
                timezone.localtime(getattr(task, "updated_at")).isoformat()
                if getattr(task, "updated_at", None)
                else None
            ),
            "created_at": (
                timezone.localtime(getattr(task, "created_at")).isoformat()
...
def write_task_backup(
    task, *, kind: str, reason: str = "save", actor_username: str = "", keep_last: int = 40
    ...
    payload = _task_backup_payload(
        task, kind=safe_kind, reason=reason, actor_username=str(actor_username or "").strip()
    )
    ...
    try:
        ...
        if team_obj:
            ...
            try:
                ids = list(
                    SessionTaskBackup.objects.filter(
                        team=team_obj, task_id=int(task_id), kind=str(safe_kind or "task")[:40]
                    )
