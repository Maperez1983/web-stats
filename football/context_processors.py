...
def _static_build_id() -> str:
    ...
    try:
        ...
        candidates = [
            ...
            base_dir / "football" / "static" / "football" / "js" / "task_builder" / "task_builder.js",
            base_dir / "football" / "static" / "football" / "css" / "task_builder" / "task_builder.css",
            base_dir / "football" / "static" / "football" / "css" / "task_builder" / "modules" / "00_shell.css",
            base_dir / "football" / "static" / "football" / "css" / "task_builder" / "modules" / "02_canvas.css",
            base_dir / "football" / "static" / "football" / "css" / "task_builder" / "modules" / "03_presentation.css",
...
def brand_theme(request):
    ...
    if raw is None:
        ...
        try:
            pref = (
                WorkspacePreference.objects.filter(workspace=workspace, key="brand_theme:v1")
                .only("id", "value")
                .first()
            )
        except Exception:
    ...
    theme = {
        ...
        "system_image_mode": str(override.get("system_image_mode") or default.get("system_image_mode") or "home")
        .strip()
        .lower(),
        ...
        "font_decoration": str(override.get("font_decoration") or default.get("font_decoration") or "none")
        .strip()
        .lower(),
...
def workspace_access(request):
    ...
    if not workspace:
        ...
        if is_admin_user:
            try:
                ...
                logger.debug(
                    "No se pudo fijar el unico workspace club para admin %s",
    ...
    try:
        ...
        can_configure_team = (
            bool(workspace_context.can_configure_active_team(request.user, workspace, active_team))
            if workspace
            else False
        )
    ...
    try:
        ...
        try:
            membership = (
                WorkspaceMembership.objects.filter(workspace=workspace, user=request.user)
        ...
        try:
            user_is_owner = bool(
                int(getattr(workspace, "owner_user_id", 0) or 0) == int(getattr(request.user, "id", 0) or 0)
            )
        except Exception:
            user_is_owner = False
