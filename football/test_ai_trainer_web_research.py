...
class AiTrainerWebResearchTests(SimpleTestCase):
    ...
    def test_html_parser_removes_script_and_keeps_readable_text(self):
        ...
        parser.feed(
            "<html><head><title>Informe rival</title><script>bad()</script></head><body><h1>Presiona alto</h1><p>Defiende en 4-4-2.</p></body></html>"
        )
                ...
                [
                    {
                        ...
                        "ok": True,
                        "title": "Rival",
                        "text": "El rival inicia corto y pierde en carril central.",
                    }
                ]
            ),
        )
    ...
    def test_browser_fetch_falls_back_to_http_when_browser_fails(self):
        with (
            mock.patch("football.web_research.fetch_web_research_browser") as browser_fetch,
            mock.patch("football.web_research.fetch_web_research") as http_fetch,
        ):
            browser_fetch.return_value = [
                {"url": "https://example.com/", "ok": False, "error": "browser_error", "method": "browser"}
            ]
            http_fetch.return_value = [
                {
    ...
    def test_compact_web_research_keeps_method(self):
        rows = compact_web_research(
            [{"url": "https://example.com/", "ok": True, "method": "browser", "title": "T", "text": "abc"}]
        )
    ...
    def test_search_web_research_applies_domain_filters(self):
        ...
        with mock.patch("football.web_research.urllib.request.urlopen", return_value=FakeResponse(html)):
            rows = search_web_research(
                "training session", preferred_domains="uefa.com", blocked_domains="other.com", max_results=4
            )
