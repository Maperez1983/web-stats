...
def _editor_document_payload(request, task):
    ...
    return {
        ...
        'engine': {
            ...
            'track_count': len(canvas_state.get('timeline') or []) if isinstance(canvas_state.get('timeline'), list) else 0,
            'keyframe_count': len(canvas_state.get('timeline') or []) if isinstance(canvas_state.get('timeline'), list) else 0,
        },
        'graphic': {
...
def session_task_export_jobs_api(request, task_id):
    ...
    kind = raw_kind if raw_kind in {choice[0] for choice in SessionTaskExportJob.KIND_CHOICES} else SessionTaskExportJob.KIND_PDF_CLUB
