...
from . import session_task_pdf_parser, task_library_services
...
ASSISTANT_KNOWLEDGE_SUPPORTED_SUFFIXES = (
    ASSISTANT_KNOWLEDGE_PDF_SUFFIXES | ASSISTANT_KNOWLEDGE_TEXT_SUFFIXES | ASSISTANT_KNOWLEDGE_IMAGE_SUFFIXES
)
...
def extract_preview_images_from_pdf(*args, **kwargs):
    ...
    if candidates:
        ...
        if good_candidates:
            selected = sorted(
                good_candidates, key=lambda item: (int(item.get("page_idx") or 0), int(item.get("seq") or 0))
            )
        else:
...
def next_session_task_order(*args, **kwargs):
    ...
    return (
        SessionTask.objects.filter(session=session, deleted_at__isnull=True).aggregate(Max("order")).get("order__max")
        or 0
    ) + 1
...
def get_or_create_library_session_with_repository(*args, **kwargs):
    ...
    if repository == LIBRARY_REPOSITORY_TRADITIONAL:
        ...
        legacy = (
            TrainingSession.objects.filter(microcycle=microcycle, focus__iexact=legacy_focus)
...
def import_library_tasks_from_pdf_advanced(*args, **kwargs):
    ...
    for idx, pdf_file in enumerate(pdf_files, start=1):
        ...
        for extra in parsed_tasks[1:]:
            ...
            extra_title = (extra_analysis.get("title") or f'{task_title} · Tarea {extra.get("segment_index") or 0}')[
                :160
            ]
            ...
            extra_task = SessionTask.objects.create(
                session=target_session,
                title=extra_title,
                block=(segment_blocks[min(segment_index - 1, len(segment_blocks) - 1)] if segment_blocks else block),
