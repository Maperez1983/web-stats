...
def is_admin_user(user):
    ...
    return bool(
        user and user.is_authenticated and (user.is_superuser or user.is_staff or role == AppUserRole.ROLE_ADMIN)
    )
...
def _workspace_group_ids(workspace):
    ...
    try:
        candidate_children = (
            Workspace.objects.filter(kind=Workspace.KIND_CLUB, is_active=True)
        ...
        for candidate in candidate_children:
            ...
            source_match = re.search(
                r"Separado automáticamente desde\s+(.+?)(?:\.|$)", candidate_notes, flags=re.IGNORECASE
            )
            source_key = _normalize_workspace_group_key(source_match.group(1).strip()) if source_match else ""
            split_parent_key = (
                _normalize_workspace_group_key(candidate_name.split("·", 1)[0].strip()) if "·" in candidate_name else ""
            )
    ...
    if can_access_platform(request.user):
        ...
        if wants_club_home:
            try:
                ...
                logger.debug(
                    "No se pudo resolver workspace club preferido para usuario %s",
                    getattr(request.user, "id", None),
                    exc_info=True,
                )
        try:
            club_ws = list(available_qs.filter(kind=Workspace.KIND_CLUB, is_active=True).order_by("id")[:2])
            if len(club_ws) == 1:
                workspace = club_ws[0]
                request.session["active_workspace_id"] = workspace.id
                return _cache_active_workspace(request, workspace)
    ...
    if single_club_fallback_enabled():
        try:
            ...
            if role and role != AppUserRole.ROLE_PLAYER:
                ...
                if len(club_ws) == 1:
                    club = club_ws[0]
                    if not WorkspaceMembership.objects.filter(workspace=club, user=request.user).exists() and int(
                        getattr(club, "owner_user_id", 0) or 0
                    ) != int(request.user.id):
            ...
            logger.debug(
                "No se pudo crear membresia viewer de fallback para usuario %s",
                getattr(request.user, "id", None),
                exc_info=True,
            )
        ...
        if role in {
            ...
            if len(club_ws) == 1:
                ...
                try:
                    ...
                    logger.debug(
                        "No se pudo crear membresia member de fallback para usuario %s",
                        getattr(request.user, "id", None),
                        exc_info=True,
                    )
                request.session["active_workspace_id"] = workspace.id
...
def workspace_team_links(workspace):
    ...
    links = sorted(
        ...
        key=lambda link: (
            (
                0
                if int(getattr(link, "workspace_id", 0) or 0) == int(workspace.id)
                and getattr(link, "is_default", False)
                else 1
            ),
...
def workspace_team_links_for_user(workspace, user):
    ...
    try:
        access_rows = list(
            WorkspaceTeamAccess.objects.filter(workspace=workspace, user=user).values_list("team_id", "is_default")
        )
    except Exception:
        access_rows = []
    if access_rows:
        ...
        return {
            ...
            for team_id in (
                StaffMember.objects.filter(
                    workspace=workspace, user=user, is_active=True, team__isnull=False
...
def user_can_access_team(request, team: "Team") -> bool:
    ...
    try:
        ...
        logger.debug(
            "No se pudo evaluar acceso platform del usuario %s", getattr(request.user, "id", None), exc_info=True
        )
    ...
    try:
        if WorkspaceTeam.objects.filter(
            team=team, workspace__is_active=True, workspace__owner_user=request.user
        ).exists():
            return True
        return WorkspaceTeam.objects.filter(
            team=team, workspace__is_active=True, workspace__memberships__user=request.user
        ).exists()
    except Exception:
        return False
...
def team_from_request_param(request):
    if not request:
        return None
    team_id = _parse_int(
        request.GET.get("team") or request.GET.get("team_id") or request.POST.get("team") or request.POST.get("team_id")
    )
    if team_id:
...
def get_active_team_for_request(request):
    ...
    if workspace and workspace.kind == Workspace.KIND_CLUB:
        ...
        if desired_team_id and int(desired_team_id) in team_lookup:
            explicit_team_param = bool(
                request and (request.GET.get("team") or request.GET.get("team_id") or request.GET.get("active_team_id"))
            )
        ...
        try:
            if request and getattr(request, "user", None) and request.user.is_authenticated and workspace:
                preferred_team_id = (
                    WorkspaceTeamAccess.objects.filter(workspace=workspace, user=request.user, is_default=True)
                    .values_list("team_id", flat=True)
                    .first()
                )
            ...
            logger.debug(
                "No se pudo resolver equipo preferido para workspace %s", getattr(workspace, "id", None), exc_info=True
            )
        default_link = next((link for link in links if getattr(link, "is_default", False)), None)
        if default_link and getattr(default_link, "team", None):
            ...
            return _cache_active_team(request, team)
        if (
            getattr(workspace, "primary_team_id", None)
            and int(getattr(workspace, "primary_team_id", 0) or 0) in team_lookup
        ):
    ...
    if (
        ...
        and getattr(request, "user", None)
        and request.user.is_authenticated
        and not can_access_platform(request.user)
    ):
        try:
        ...
        if explicit_team:
            try:
                ...
                logger.debug(
                    "No se pudo guardar active_team_id %s en sesion", getattr(explicit_team, "id", None), exc_info=True
                )
        ...
        try:
            ...
            logger.debug(
                "No se pudo cargar accesos directos a equipos para usuario %s",
                getattr(request.user, "id", None),
                exc_info=True,
            )
        if not team_ids:
            try:
                team_ids.update(
                    WorkspaceTeam.objects.filter(
                        workspace__is_active=True, workspace__owner_user=request.user
                    ).values_list("team_id", flat=True)
                )
            except Exception:
                logger.debug(
                    "No se pudo cargar equipos por ownership para usuario %s",
                    getattr(request.user, "id", None),
                    exc_info=True,
                )
        if not team_ids:
            try:
                team_ids.update(
                    WorkspaceTeam.objects.filter(
                        workspace__is_active=True, workspace__memberships__user=request.user
                    ).values_list("team_id", flat=True)
                )
            except Exception:
                logger.debug(
                    "No se pudo cargar equipos por membresia para usuario %s",
                    getattr(request.user, "id", None),
                    exc_info=True,
                )
        ...
        if len(team_ids) == 1:
            ...
            if team:
                try:
                    ...
                    logger.debug(
                        "No se pudo guardar unico active_team_id %s en sesion", getattr(team, "id", None), exc_info=True
                    )
            return _cache_active_team(request, team)
        if team_ids:
            try:
            ...
            if fallback_team_id:
                ...
                if team:
                    try:
                        ...
                        logger.debug(
                            "No se pudo guardar fallback active_team_id %s en sesion",
                            getattr(team, "id", None),
                            exc_info=True,
                        )
                    return _cache_active_team(request, team)
...
def can_configure_active_team(user, workspace, team=None):
    ...
    try:
        ...
        logger.debug(
            "No se pudo resolver alcance de equipos para configurar workspace %s",
            getattr(workspace, "id", None),
            exc_info=True,
        )
        team_ids = set()
    ...
    try:
        ...
        logger.debug(
            "No se pudo comprobar si el workspace %s es mono-equipo", getattr(workspace, "id", None), exc_info=True
        )
    return False
