...
class WorkspaceSchedulePayloadTests(TestCase):
    def test_build_workspace_schedule_payload_orders_and_marks_status(self):
        ...
        Match.objects.create(
            season=season, group=group, home_team=team, away_team=rival, date=today, round="J1", location="Campo"
        )
    ...
    def test_build_local_next_match_payload_falls_back_to_latest_match(self):
        ...
        Match.objects.create(
            season=season, group=group, home_team=rival, away_team=team, date=yesterday, round="J0", location="Campo"
        )
    ...
    def test_next_match_payload_is_usable_rejects_missing_round_or_placeholder_opponent(self):
        self.assertFalse(
            match_payload_services.next_match_payload_is_usable({"status": "next", "opponent": "Rival FC"})
        )
        self.assertFalse(
            match_payload_services.next_match_payload_is_usable(
                {"status": "next", "round": "J1", "opponent": "Rival por confirmar"}
            )
        )
        self.assertTrue(
            match_payload_services.next_match_payload_is_usable(
                {"status": "next", "round": "J1", "opponent": "Rival FC"}
            )
        )
