...
def seed_season_history(apps, schema_editor):
    ...
    for workspace in Workspace.objects.filter(active_season__isnull=False).only("id", "active_season_id"):
        ...
        team_ids = list(WorkspaceTeam.objects.filter(workspace_id=workspace.id).values_list("team_id", flat=True))
        for team_id in team_ids:
            ...
            existing = set(
                WorkspaceSeasonPlayer.objects.filter(
                    season_id=season_id, player_id__in=[pid for pid, _active in player_ids]
    ...
    for membership in WorkspaceSeasonPlayer.objects.select_related("player").only(
        "id", "is_confirmed", "status", "player__is_active"
    ):
...
class Migration(migrations.Migration):
    ...
    operations = [
        ...
        migrations.AlterField(
            ...
            field=models.CharField(
                choices=[
                    ("manual", "Manual / base local"),
                    ("rfaf", "RFAF"),
                    ("universo_rfaf", "Universo RFAF"),
                    ("lapreferente", "La Preferente"),
                ],
                default="manual",
                max_length=32,
            ),
