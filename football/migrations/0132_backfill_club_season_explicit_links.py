...
def backfill_club_seasons(apps, schema_editor):
    ...
    for workspace in Workspace.objects.filter(kind="club"):
        ...
        specs = [
            (
                Match.objects.filter(
                    Q(home_team_id__in=team_ids) | Q(away_team_id__in=team_ids), club_season__isnull=True
                ),
                "date",
            ),
            (
                TrainingSession.objects.filter(microcycle__team_id__in=team_ids, club_season__isnull=True),
                "session_date",
            ),
        ...
        task_qs = SessionTask.objects.filter(
