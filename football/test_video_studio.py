    ...
    def test_video_studio_export_job_create_and_cancel(self):
        ...
        with patch("football.views.shutil.which", return_value=None):
            create = self.client.post(
                ...
                data=json.dumps(
                    {"video_id": self.video.id, "items": [{"clip_id": c1.id}, {"clip_id": c2.id}], "title": "Job"}
                ),
                content_type="application/json",
            )
...
class VideoStudioPersonalLibraryTests(TestCase):
    def setUp(self):
        ...
        self.team = Team.objects.create(
            name="Equipo VS Personal", slug="equipo-vs-personal", group=group, is_primary=True
        )
        self.workspace = Workspace.objects.create(
    ...
    def test_personal_video_can_create_clip_timeline_project_and_assign_to_team(self):
        ...
        ev_save = self.client.post(
            reverse("analysis-video-studio-timeline-save-api"),
            data=json.dumps(
                {"video_id": self.video.id, "time_s": 1.2, "kind": "tag", "label": "Marca", "color": "#22d3ee"}
            ),
            content_type="application/json",
        )
        self.assertEqual(ev_save.status_code, 200)
        self.assertTrue(ev_save.json().get("ok"))
        self.assertTrue(
            VideoTimelineEvent.objects.filter(video=self.video, team__isnull=True, owner_user=self.user).exists()
        )
        ...
        self.assertTrue(
            VideoTelestrationProject.objects.filter(video=self.video, team__isnull=True, owner_user=self.user).exists()
        )
