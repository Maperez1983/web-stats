...
from football.models import ConvocationRecord, Match, MatchEvent, Player, PlayerInjuryRecord, PlayerStatistic, Team
...
def get_active_injury_player_ids(player_ids):
    ...
    try:
        return set(
            PlayerInjuryRecord.objects.filter(player_id__in=normalized_ids, is_active=True)
...
def _team_match_queryset(primary_team):
    ...
    if alias_ids is None:
        alias_ids = []
        # Producto multicategoría: solo permitimos alias por nombre (La Preferente legacy)
        # en el equipo principal. En categorías secundarias es demasiado fácil mezclar datos
        # cuando comparten nombre/grupo por clonaciones (Senior vs Prebenjamín).
        if bool(getattr(primary_team, "is_primary", False)):
            ...
            if primary_group_id:
                candidate_qs = Team.objects.exclude(id=primary_team.id).only(
                    "id", "name", "category", "game_format", "group_id"
                )
                ...
                if primary_category:
                    try:
                        candidate_qs = candidate_qs.filter(
                            Q(category=primary_category) | Q(category="") | Q(category__isnull=True)
                        )
                    except Exception:
                        candidate_qs = candidate_qs
                ...
                if primary_game_format:
                    try:
                        candidate_qs = candidate_qs.filter(
                            Q(game_format=primary_game_format) | Q(game_format="") | Q(game_format__isnull=True)
                        )
                    except Exception:
                        candidate_qs = candidate_qs
                ...
                for candidate in candidate_qs:
                    ...
                    fuzzy_same_team = bool(
                        ...
                        and (primary_lookup in candidate_lookup or candidate_lookup in primary_lookup)
                    )
    ...
    if isinstance(cached_extra_ids, dict):
        ...
        event_match_ids = list(
            MatchEvent.objects.filter(player__team=primary_team).values_list("match_id", flat=True).distinct()
        )
        convocation_match_ids = list(
            ConvocationRecord.objects.filter(team=primary_team, match_id__isnull=False)
            .values_list("match_id", flat=True)
            .distinct()
        )
        player_stat_match_ids = list(
            PlayerStatistic.objects.filter(player__team=primary_team, match_id__isnull=False)
            .values_list("match_id", flat=True)
            .distinct()
        )
        cache.set(
...
def get_previous_match(primary_team, reference_match=None):
    ...
    if reference_match and reference_match.date:
        previous = (
            qs.exclude(id=reference_match.id).filter(date__lt=reference_match.date).order_by("-date", "-id").first()
        )
        if previous:
            return previous
    today = timezone.localdate()
...
def get_sanctioned_player_ids_from_previous_round(primary_team, reference_match=None):
    ...
    sanctioned_ids = set()
    events = (
        confirmed_events_queryset().filter(match=previous_match, player__team=primary_team).select_related("player")
    )
    for event in events:
