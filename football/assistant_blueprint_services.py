...
from . import session_import_services, task_library_services
...
def create_task_sheet_blueprint_from_document(
    ...
    if not desc_lines and raw_lines:
        ...
        try:
            desc_lines = [
                str(v or "").strip() for v in raw_lines[idx_title + 1 : idx_title + 20] if str(v or "").strip()
            ]
        except Exception:
            desc_lines = []
    ...
    dimensions = (
        sanitize(space_raw.replace("X", "x").replace(" ", ""), multiline=False, max_len=24) if space_raw else ""
    )
    ...
    objective_src = (
        " · ".join(objective_bullets[:2]).strip()
        or " ".join(beh_lines[:2]).strip()
        or " ".join(desc_lines[:2]).strip()
        or title
    )
    ...
    text_for_infer = "\n".join(
        [title] + list(desc_lines[:40]) + list(beh_lines[:30]) + list(cons_lines[:30]) + list(struct_lines[:30])
    )
    ...
    tpl = {
        ...
        **(
            {"dimensions": f"{dimensions} m" if dimensions and not dimensions.lower().endswith("m") else dimensions}
            if dimensions
            else {}
        ),
...
def _read_document_file_bytes(doc) -> bytes:
    try:
        ...
        try:
            ...
            logger.debug(
                "No se pudo abrir fichero del documento %s antes de leerlo", getattr(doc, "id", None), exc_info=True
            )
        try:
