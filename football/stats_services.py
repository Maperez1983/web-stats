...
def preferred_event_source_by_match(primary_team, scope=None):
    ...
    if scope_value not in {Match.CONTEXT_LEAGUE, Match.CONTEXT_TOURNAMENT, Match.CONTEXT_FRIENDLY, "all", ""}:
        scope_value = ""
    team_events = MatchEvent.objects.filter(player__team=primary_team).filter(
        Q(source_file="registro-acciones") | ~Q(system="touch-field")
    )
    ...
    registro_match_ids = set(
        team_events.filter(source_file="registro-acciones").values_list("match_id", flat=True).distinct()
    )
...
def compute_team_metrics(primary_team, scope=Match.CONTEXT_LEAGUE, request=None):
    ...
    payload = {
        ...
        "top_event_types": [
            ...
            for etype, count in Counter(event.event_type for event in events).most_common(5)
        ],
        "top_results": [
            {"result": result, "count": count}
            for result, count in Counter(event.result for event in events).most_common(5)
        ],
    }
    if not date_start and not date_end:
...
def compute_team_metrics_for_match(match, primary_team=None):
    ...
    return {
        ...
        "top_event_types": [
            {"event": etype, "count": count}
            for etype, count in Counter(event.event_type for event in events).most_common(6)
        ],
        "top_results": [
            {"result": result, "count": count}
            for result, count in Counter(event.result for event in events).most_common(6)
        ],
    }
