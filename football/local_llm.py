...
def local_llm_config():
    ...
    model = (
        str(os.getenv("AI_TRAINER_LOCAL_LLM_MODEL") or os.getenv("OLLAMA_MODEL") or DEFAULT_QWEN_MODEL).strip()
...
def build_ai_trainer_context(
    *, team_name, profile, phase, goal, signals, club_model, learning_memory, suggestions, proposals, web_research=None
):
    ...
    return {
        ...
        "game_model": {
            ...
            "attack_principles": _compact_list(
                model.get("attack_principles") if isinstance(model.get("attack_principles"), list) else [], limit=8
            ),
            "defense_principles": _compact_list(
                model.get("defense_principles") if isinstance(model.get("defense_principles"), list) else [], limit=8
            ),
            "transition_principles": _compact_list(
                model.get("transition_principles") if isinstance(model.get("transition_principles"), list) else [],
                limit=8,
            ),
            "behavior_rules": _compact_list(
                model.get("behavior_rules") if isinstance(model.get("behavior_rules"), list) else [], limit=8
            ),
        },
...
def ai_trainer_senior_local_advice(
    *,
    team_name,
    profile,
    phase,
