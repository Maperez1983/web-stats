...
def _build_task_date_haystack(task, analysis_meta=None):
    sheet = (
        analysis_meta.get("task_sheet")
        if isinstance(analysis_meta, dict) and isinstance(analysis_meta.get("task_sheet"), dict)
        else {}
    )
    return "\n".join(
        [
...
def _apply_analysis_to_task(task, analysis):
    ...
    description_html_clean = (
        _sanitize_task_rich_html(description_html_raw)
        if description_html_raw
        else _rich_html_from_plain_text(description_plain)
    )
    coaching_html_clean = (
        _sanitize_task_rich_html(coaching_html_raw)
        if coaching_html_raw
        else _rich_html_from_plain_text(str(analysis.get("coaching_points") or ""))
    )
    rules_html_clean = (
        _sanitize_task_rich_html(rules_html_raw)
        if rules_html_raw
        else _rich_html_from_plain_text(str(analysis.get("confrontation_rules") or ""))
    )
...
def _extract_section_block(text, section_aliases):
    ...
    for idx in range(start, len(lines)):
        ...
        if re.match(r"^(?:tarea|ejercicio|task|drill)\s*(?:n[\.\s]*[oº°])?\s*\d{1,2}\b", folded):
            ...
            break
        if any(
            folded.startswith(token.replace(" ", "")) or folded.startswith(token)
...
def _extract_task_sheet_from_pdf(text, detected_materials=None, template_key="generic"):
    ...
    if template_key == "sesion_sheet_compact" and not description:
        ...
        if lines:
            ...
            for idx, ln in enumerate(lines[:70]):
                folded = _normalize_folded_text(ln)
                if (
                    folded.startswith("consignas")
                    or folded.startswith("dosificacion")
    ...
    return {
        "description": _polish_spanish_text(
            _repair_joined_words_text((description or "")[:1400]), multiline=True, max_len=1200
        ),
...
def _suggest_task_from_pdf(pdf_text):
    ...
    if title and title_from_marker:
        ...
        if isinstance(idx, int) and 0 <= idx < len(lines):
            ...
            wants_more = (len(title.strip()) < 22) or tail.endswith(
            ...
            if wants_more:
                ...
                for nxt in lines[idx + 1 : idx + 4]:
                    ...
                    if len(extras) >= 1 and not combined_tail.endswith(
                        (" de", " del", " la", " el", " los", " las", " con", " sin", " para", " por")
                    ):
                        break
                    if len(extras) >= 2:
    ...
    has_consignas = bool(
        re.search(r"(?im)^\s*consignas\s*[:\-]?\s*$", text) or re.search(r"(?i)\bconsignas\s*[:\-]\s*\S", text)
    )
    has_rules = bool(
        re.search(r"(?im)^\s*(?:reglas|normas)\s*[:\-]?\s*$", text)
        or re.search(r"(?i)\b(?:reglas|normas)\s*[:\-]\s*\S", text)
    )
    ...
    objective_tags = _detect_keyword_tags(
        " ".join([objective, coaching_points, confrontation_rules]), TASK_OBJECTIVE_KEYWORDS
    )
    ...
    try:
        ...
        task_sheet["coaching_html"] = (
            _rich_html_from_plain_text(coaching_points) if str(coaching_points or "").strip() else ""
        )
        task_sheet["rules_html"] = (
            _rich_html_from_plain_text(confrontation_rules) if str(confrontation_rules or "").strip() else ""
        )
    except Exception:
        pass
    ...
    analysis = {
        "title": _polish_spanish_text(
            _repair_joined_words_text((title or "Tarea desde PDF")[:220]), multiline=False, max_len=160
        ),
...
def _split_pdf_into_task_sections(pdf_text):
    ...
    for i, start in enumerate(boundaries):
        ...
        has_load_or_task_data = (
            "dosificacion" in folded or "tiempototaldetrabajo" in folded or "consignas" in folded or "tarea" in folded
        )
    ...
    if sections:
        ...
        has_first_marker = any(
            any(token in _normalize_folded_text(sec) for token in ("tarea1", "tarea 1", "primera tarea"))
            for sec in sections
        )
...
def _suggest_blocks_for_session_pdf_segments(parsed_tasks, fallback_block):
    ...
    is_sessionish = any(
        "microciclo" in seg or "mesociclo" in seg or "materialdeentrenamiento" in seg for seg in segments[:2]
    )
    has_activation_ball = any(("activacion" in seg and "balon" in seg) for seg in segments[:3])
    has_conditioning = any(
        any(tok in seg for tok in ("acondicionamiento", "trabajofisico", "fuerza", "metabol")) for seg in segments[:3]
    )
    ...
    for seg in segments:
        if any(
            tok in seg
            for tok in ("estiramientos", "vueltaalacalma", "partefinal", "parte final", "recuperacion", "recuperación")
        ):
            blocks.append(SessionTask.BLOCK_RECOVERY)
            continue
        if any(
            tok in seg for tok in ("acondicionamiento", "trabajofisico", "circuitofuerza", "intermitente", "metabol")
        ):
