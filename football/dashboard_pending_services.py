...
def build_team_pending_cards(primary_team, weekly_brief=None, weekly_brief_builder=None):
    ...
    future_sessions = list(
        TrainingSession.objects.filter(microcycle__team=primary_team, session_date__gte=today)
    ...
    if rival_name:
        has_ready_manual_report = (
            RivalAnalysisReport.objects.filter(team=primary_team, status=RivalAnalysisReport.STATUS_READY)
        ...
        has_exported_pptx = (
            AnalysisVideoReport.objects.filter(team=primary_team)
            .filter(needle_q)
