...
def _normalized_match_key(match, primary_team):
    opponent = (
        ...
        if match.home_team == primary_team and match.away_team
            ...
            match.home_team.display_name if match.away_team == primary_team and match.home_team else "Rival desconocido"
        )
    )
    return (
...
def run_stats_audit(primary_team):
    ...
    standings = list(TeamStanding.objects.filter(team=primary_team).select_related("season", "group").order_by("-id"))
    standing = standings[0] if standings else None
    matches = list(_team_match_queryset(primary_team).select_related("home_team", "away_team").order_by("date", "id"))
