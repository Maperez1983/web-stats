    ...
    *,
    ...
    for task in tasks:
            ...
            confidence = analysis_confidence_scores(
                {
                    ...
                    "phase_tags": (
                        analysis_meta.get("phase_tags") if isinstance(analysis_meta.get("phase_tags"), list) else []
                    ),
        ...
        task.exercise_types = normalize_tags(
            (analysis_meta.get("exercise_types") if isinstance(analysis_meta.get("exercise_types"), list) else [])
        )
        ...
        quality_checks = [
            (
                "Sin descripción",
                _truthy(task_sheet.get("description"))
                or _truthy(task.analysis_summary)
                or _truthy(task.objective_summary),
            ),
            ...
            (
                "Sin espacio",
                _truthy(task_sheet.get("space")) or _truthy(task_sheet.get("dimensions")) or _truthy(meta.get("space")),
            ),
            (
                "Sin jugadores",
                _truthy(task_sheet.get("players"))
                or _truthy(meta.get("players_distribution"))
                or _truthy(meta.get("players")),
            ),
            ("Sin material", _truthy(task_sheet.get("materials")) or bool(task.detected_materials)),
            (
                "Sin variantes",
                _truthy(meta.get("progression"))
                or _truthy(meta.get("progression_html"))
                or _truthy(meta.get("regression"))
                or _truthy(meta.get("regression_html")),
            ),
            (
                "Sin criterio de éxito",
                _truthy(meta.get("success_criteria")) or _truthy(meta.get("success_criteria_html")),
            ),
        ]
    ...
    quality_group_rows = [
        {
            ...
            "count": len([item for item in tasks if getattr(item, "needs_review", False)]),
        },
        {
            "key": "validated",
            "label": "Validadas",
            "count": len([item for item in tasks if not getattr(item, "needs_review", False)]),
        },
    ]
...
def filter_task_library(tasks, *, library_view, library_key):
    ...
    filtered.sort(
        key=lambda item: (
            getattr(item, "reference_date", None)
            or getattr(getattr(item, "session", None), "session_date", None)
            or date.min,
...
def filter_task_library_advanced(
    ...
    *,
    ...
    if sort_key == "title":
        filtered.sort(
            key=lambda item: (str(getattr(item, "title", "") or "").lower(), int(getattr(item, "id", 0) or 0))
        )
        return filtered
