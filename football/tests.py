...
from football import workspace_context
...
class Pitch3dAssetContextTests(SimpleTestCase):
    def test_task_pitch3d_asset_context_points_to_existing_stadium_model(self):
        context = football_views._task_pitch3d_asset_context()
        stadium_src = str(context.get("pitch3d_stadium_model_src") or "")
        self.assertIn("stadium_architectural_complete.glb", stadium_src)
        self.assertTrue(finders.find("football/models/pitch3d/stadium_architectural_complete.glb"))
        self.assertTrue(finders.find("football/images/pitch3d/stadium_taskboard_top_h.png"))
        self.assertTrue(finders.find("football/images/pitch3d/stadium_taskboard_overlay_h.png"))

    @patch("football.render_engine.renderer_3d.render_task_detail_3d_scene_png")
    def test_task_preview_3d_returns_empty_when_scene_snapshot_fails(self, render_scene):
        render_scene.return_value = None

        preview = task_renderer_3d.render_task_preview_3d(
            {"version": "5.3.0", "objects": []},
            task=SimpleNamespace(title="Tarea 3D"),
            team=SimpleNamespace(name="Club 3D", display_name="Club 3D"),
...
class SystemGuardTests(TestCase):
    def setUp(self):
        ...
        self.workspace = Workspace.objects.create(
            name="Guard Workspace", primary_team=self.team, owner_user=self.workspace_owner
        )
        self.user = get_user_model().objects.create_user(
    ...
    @patch(
        "football.system_guard.local_llm_config",
        return_value={
            "enabled": True,
            "provider": "ollama",
            "model": "qwen3:8b",
    ...
    @patch(
        "football.system_guard._probe_ollama",
        return_value={
            "enabled": True,
            "ok": False,
            "reachable": False,
    ...
    @patch(
        "football.system_guard.run_system_healthcheck",
        return_value={
            "ok": True,
    ...
    @patch(
        "football.system_guard._execute_tools",
        return_value=[
            {
                "tool": "auto_fix",
    ...
    def test_planner_selects_visual_page_inspection_for_task_ficha_questions(self):
        plan = system_guard._plan_tools(
            ...
            page_context={
                ...
                "browser_target_url": "https://app.example.com/coach/sesiones/tarea/90/",
            },
        )
    ...
    def test_infer_intent_marks_visual_issue_as_repair_when_user_asks_to_solve_it(self):
        intent = system_guard._infer_intent("Por que no se visualiza bien el estadio 3d y si lo puedes solucionar")
    ...
    @patch(
        "football.system_guard._browser_visual_openai_analysis",
        return_value={
            "enabled": True,
            "provider": "openai",
            "model": "gpt-4o-mini",
    ...
    def test_catalog_candidates_match_question_terms(self):
        ...
        self.assertTrue(
            any(row["key"] == "widget_library_ai_navigation_keywords" and row["auto_apply"] for row in widget_rows)
        )

    def test_catalog_candidates_include_widget_and_pitch3d_domains(self):
        widget_rows = system_guard._catalog_candidates_for_question("El widget Ollana no aparece abajo a la derecha")
    ...
    def test_execute_catalog_code_intervention_applies_exact_patch_in_temp_repo(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with (
                patch.object(system_guard, "CODE_INTERVENTION_CATALOG", catalog),
                patch.object(system_guard.app_settings, "BASE_DIR", base_dir),
            ):
    ...
    def test_execute_controlled_technical_operation_applies_auto_apply_catalog_candidate(self):
        operation = {
            ...
            "catalog_candidates": [
                {"key": "widget_library_ai_navigation_keywords", "title": "Widget nav", "auto_apply": True}
            ],
        }
        executed_tools = [
            {
        ...
        operator = system_guard._build_real_code_operator(
            ...
            technical_execution={
                ...
                "applied_interventions": [
                    {"ok": True, "candidate_key": "pitch3d_trigger_and_modal_flow", "title": "Pitch3D"}
                ],
            },
            repository_operator={
            ...
            executions=[
                ...
                {"tool": "check_critical_routes", "ok": True, "result": {"ok": True, "ok_count": 12, "failing": []}},
                {
                    ...
                    "result": {
                        ...
                        "healthz": {"ok": True, "status_code": 200},
                    },
                },
            ],
    ...
    def test_build_self_healing_operator_prefers_catalog_replay_when_ready(self):
        operator = system_guard._build_self_healing_operator(
            ...
            technical_operation={
                "kind": "technical_operation",
                "catalog_candidates": [
                    {"key": "pitch3d_trigger_and_modal_flow", "title": "Pitch3D", "auto_apply": True}
                ],
            },
            technical_execution={"publish_ready": True},
    ...
    @patch.dict(
        os.environ,
        {"APP_PUBLIC_BASE_URL": "https://app.example.com", "RENDER_EXTERNAL_HOSTNAME": "app.example.com"},
        clear=False,
    )
    @patch.dict(
        os.environ,
    ...
    def test_external_connectors_snapshot_reports_core_channels(self, _llm):
        snapshot = system_guard._external_connectors_snapshot(
            page_context={
                "page": "dashboard-home",
                "workspace_id": self.workspace.id,
                "team_id": self.team.id,
    ...
    def test_external_connectors_snapshot_reports_render_api(
        self, mock_list_render_services, mock_inspect_render_service
    ):
        ...
        mock_inspect_render_service.side_effect = [
            {
                ...
                "deploys": {
                    "enabled": True,
                    "count": 1,
                    "summary": {"count": 1, "latest": {"id": "dep-1", "status": "live"}, "items": []},
                    "reason": "",
                },
            },
            {
                ...
                "deploys": {
                    "enabled": True,
                    "count": 1,
                    "summary": {"count": 1, "latest": {"id": "dep-2", "status": "live"}, "items": []},
                    "reason": "",
                },
            },
        ]
    ...
    def test_external_connectors_snapshot_reports_database_readonly(self, mock_inspect_database_readonly):
        mock_inspect_database_readonly.return_value = {
            ...
            "tables": [
                {
                    "name": "football_sesstask",
                    "row_count": 12,
                    "columns": ["id", "title"],
                    "duplicate_values": {"title": [{"value": "Juego ludico", "count": 2}]},
                },
    ...
    def test_database_inspector_prioritizes_context_tables_and_detects_duplicates(
        self, mock_connections, _mock_app_config
    ):
        cursor = Mock()
        cursor_context = Mock()
        ...
        connection.introspection.get_table_description.side_effect = lambda cur, table: {
            "football_sessiontask": [
                SimpleNamespace(name="id"),
                SimpleNamespace(name="title"),
                SimpleNamespace(name="slug"),
            ],
    ...
    def test_safe_command_executor_snapshot_respects_code_permissions(self):
        snapshot = system_guard._safe_command_executor_snapshot(
            page_context={
                "page": "dashboard-home",
    ...
    def test_permission_profile_marks_admin_total_operator(self):
        profile = system_guard._permission_profile(
            page_context={
                "is_admin_user": True,
                "can_manage_guard": True,
                "can_operate_guard_code": True,
    ...
    def test_execute_controlled_technical_operation_applies_catalog_fix_and_revalidates(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with (
                ...
                patch.object(system_guard.app_settings, "BASE_DIR", base_dir),
                patch(
                    "football.system_guard._run_operator_validation",
                    return_value={"ok": True, "action": "operator_validation"},
                ),
            ):
                result = system_guard._execute_controlled_technical_operation(
                    operation,
    ...
    def test_system_guard_can_dedupe_session_tasks_across_workspace(self, *_mocks):
        ...
        self.assertEqual(
            SessionTask.objects.filter(
                deleted_at__isnull=True, session__microcycle__team__in=[self.team, other_team]
            ).count(),
            2,
        )
        self.assertEqual(
            SessionTask.objects.filter(
                deleted_at__isnull=False, session__microcycle__team__in=[self.team, other_team]
            ).count(),
            2,
        )
    ...
    def test_detect_proactive_incidents_builds_runtime_detector_rows(self):
        ...
        system_guard._store_pref_value(
            self.workspace,
            system_guard.SNAPSHOTS_PREF_KEY,
            [
                {
                    "status": "watch",
                    "blockers": 1,
                    "warnings": 1,
                    "issue_ids": ["ollama_unreachable"],
                    "llm_state": "down",
                },
            ],
        )
    ...
    def test_run_proactive_guard_cycle_creates_and_executes_safe_tasks(self, _exec, *_mocks):
        with patch(
            "football.system_guard._detect_proactive_incidents",
            return_value=[
                {
                    "detector": "ollama_unreachable",
                    "severity": "warning",
    ...
    def test_run_system_guard_chat_can_create_session_from_conversation(self, *_mocks):
        ...
        self.assertEqual(
            response["intelligence_os"]["layers"]["runtime"]["latest_session"]["focus"], "Finalización + presión"
        )
    ...
    def test_run_system_guard_chat_can_execute_action_chain(self, *_mocks):
        ...
        self.assertEqual(
            response["intelligence_os"]["layers"]["conversation"]["request_contract"]["interaction_mode"],
            "composite_operator",
        )
    ...
    def test_execute_create_session_action_requires_management_permission(self):
        ...
        self.assertTrue(result["permission_required"])
        self.assertFalse(
            TrainingSession.objects.filter(microcycle__team=self.team, focus="Finalización + presión").exists()
        )
    ...
    def test_observability_summary_aggregates_trends_metrics_and_memory(self):
        ...
        system_guard._append_audit_log(
            self.workspace,
            {
        ...
        system_guard._append_incident_ledger(
            self.workspace,
            {
        ...
        system_guard._store_pref_value(
            self.workspace,
            system_guard.OBJECTIVE_MEMORY_PREF_KEY,
            [
                {
    ...
    def test_observability_summary_exposes_scheduled_state(self):
        system_guard._store_pref_value(
            self.workspace,
            system_guard.SCHEDULED_GUARD_STATE_PREF_KEY,
            {
    ...
    def test_trigger_remote_deploy_calls_configured_connector(self, mock_urlopen):
        class Resp:
            ...
            def read(self):
                return json.dumps({"id": "dep-77", "status": "queued"}).encode("utf-8")
    ...
    def test_trigger_remote_rollback_calls_configured_connector(self, mock_urlopen):
        class Resp:
            ...
            def read(self):
                return json.dumps({"id": "rb-44", "status": "queued"}).encode("utf-8")
    ...
    def test_objective_orchestrator_tracks_long_running_goal(self):
        row = system_guard._update_objective_memory(
            ...
            planner={
                "task": {"scope": "code", "kind": "repair", "title": "Corregir flujo 3D"},
                "runbook": {"key": "safe_repair"},
            },
            technical_operation={"kind": "technical_operation"},
            technical_execution={
                "status": "running",
                "completed_phases": ["triage", "inspect_repo"],
                "next_step": "Aplicar patch",
            },
    ...
    @patch(
        "football.system_guard._trigger_remote_rollback",
        return_value={"ok": True, "status": "queued", "rollback_id": "rb-99"},
    )
    ...
    def test_build_silent_operator_state_exposes_top_priority(self):
        system_guard._enqueue_task(
            self.workspace,
            system_guard._new_task_entry(
                detector="route_failure",
    ...
    @patch(
        "football.system_guard.run_system_guard",
        return_value={
            "ok": True,
    ...
    @patch(
        "football.system_guard.run_continuous_operator_cycle",
        return_value={
            "ok": True,
    ...
    def test_run_continuous_operator_cycle_stops_when_control_flag_is_set(self):
        system_guard._store_operator_control(
            self.workspace,
            {
                "stop_requested": True,
    ...
    @patch(
        "football.system_guard._external_connectors_snapshot",
        return_value={
            "items": [
                {
                    "key": "repository",
    ...
    def test_run_system_guard_chat_exposes_code_operator_mode_for_feature_request(self, _snapshot, _exec, *_mocks):
        result = system_guard.run_system_guard_chat(
        ...
        self.assertIn(
            response["deployment_guard"]["status"],
            {
                "pre_deploy_check",
        ...
        self.assertIn(
            response["self_healing"]["strategy"],
            {"catalog_autofix_replay", "memory_guided_repair", "repeated_incident_repair"},
        )
        ...
        self.assertEqual(
            response["intelligence_os"]["layers"]["execution"]["repair_commander"]["status"], "publish_ready"
        )
        ...
        self.assertEqual(
            {
                row["key"]: row
                for row in response["intelligence_os"]["layers"]["execution"]["external_connectors"]["items"]
            }["repository"]["status"],
            "connected",
        )
        self.assertGreaterEqual(
            response["intelligence_os"]["layers"]["execution"]["safe_command_executor"]["allowed_count"], 4
        )
        ...
        self.assertEqual(
            response["intelligence_os"]["layers"]["supervision"]["autonomy_policy"]["mode"], "technical_operator"
        )
        ...
        self.assertTrue(response["intelligence_os"]["layers"]["supervision"]["external_connectors_ready"])
        self.assertGreaterEqual(response["intelligence_os"]["layers"]["supervision"]["safe_executor_allowed_count"], 4)
        self.assertIn(
            response["intelligence_os"]["layers"]["supervision"]["release_status"],
            {"ready_for_release_check", "published_verified", "monitoring", "regression_detected"},
        )
        self.assertIn(
            response["intelligence_os"]["layers"]["supervision"]["deployment_status"],
            {
                "pre_deploy_check",
                "pending_release_window",
                "deployment_verified",
        ...
        self.assertIn(
            response["intelligence_os"]["layers"]["incident_commander"]["status"],
            {"stable", "running", "blocked", "awaiting_operator", "regression_detected"},
        )
        self.assertTrue(
            response["intelligence_os"]["layers"]["mission_control"]["autonomy"]["real_code_execution_live"]
        )
    ...
    def test_run_system_guard_chat_exposes_agent_registry_planner_and_evaluator(self, _health, _cfg, _exec):
        ...
        self.assertTrue(response["intelligence_os"]["layers"]["execution"]["tool_registry"]["items"])
    ...
    def test_run_system_guard_chat_returns_navigation_action_and_silent_operator_state(self, *_mocks):
        ...
        self.assertEqual(
            response["intelligence_os"]["layers"]["mission_control"]["role"], "central_system_intelligence"
        )
        ...
        self.assertEqual(
            response["intelligence_os"]["layers"]["execution"]["action_executor"]["active_kind"], "navigate_module"
        )
        self.assertTrue(response["intelligence_os"]["layers"]["supervision"]["autonomy_controller"]["embedded"])
        ...
        self.assertEqual(
            response["intelligence_os"]["layers"]["policy_decisions"]["requested_action"], "navigate_modules"
        )
        self.assertTrue(
            any(
                str(row.get("label") or "") == "Analizar partido activo"
                for row in (system_guard._contextual_flow_actions({"page": "match-hub", "match_id": 99}) or [])
    ...
    def test_run_system_guard_chat_exposes_live_workflow_for_selected_session_and_task(self, *_mocks):
        ...
        self.assertEqual(
            response["intelligence_os"]["layers"]["mission_control"]["active_mission"]["task_kind"], "diagnose"
        )
        ...
        self.assertTrue(
            any(
                str(row.get("label") or "") == "Analizar sesión abierta"
                for row in (response.get("ui_actions") or [])
                if isinstance(row, dict)
            )
        )
        self.assertTrue(
            any(
                str(row.get("label") or "") == "Revisar tarea abierta"
                for row in (response.get("ui_actions") or [])
                if isinstance(row, dict)
            )
        )
        self.assertTrue(
            any(
                str(row.get("label") or "") == "Optimizar sesión actual"
                for row in (response.get("ui_actions") or [])
                if isinstance(row, dict)
            )
        )
        self.assertTrue(
            any(
                str(row.get("label") or "") == "Revisar tarea actual"
                for row in (response.get("ui_actions") or [])
                if isinstance(row, dict)
            )
        )
    ...
    def test_run_system_guard_chat_can_create_microcycle(self, *_mocks):
        ...
        self.assertEqual(
            response["intelligence_os"]["layers"]["policy_decisions"]["requested_action"], "create_microcycle"
        )
        self.assertEqual(
            response["intelligence_os"]["layers"]["execution"]["action_executor"]["active_kind"], "create_microcycle"
        )
    ...
    def test_run_system_guard_chat_can_create_match_and_module_runbook(self, *_mocks):
        ...
        season = Season.objects.create(
            competition=competition, name="2026/2027", start_date=date(2026, 7, 1), is_current=True
        )
        ...
        self.assertEqual(
            response["intelligence_os"]["layers"]["silent_operator_runtime"]["module_runbook"]["runbook"],
            "user_execution",
        )
    ...
    def test_run_system_guard_chat_can_create_convocation(self, *_mocks):
        ...
        system_guard.ensure_player_season_membership(
            club_season, player, team=self.team, confirmed=True, status="confirmed"
        )
        result = system_guard.run_system_guard_chat(
        ...
        self.assertEqual(
            response["intelligence_os"]["layers"]["policy_decisions"]["requested_action"], "create_convocation"
        )
    ...
    def test_run_system_guard_chat_can_prepare_rival_analysis(self, *_mocks):
        ...
        self.assertEqual(
            response["intelligence_os"]["layers"]["policy_decisions"]["requested_action"], "create_rival_analysis"
        )
    ...
    def test_run_system_guard_chat_can_create_session_bundle(self, *_mocks):
        ...
        self.assertEqual(
            response["intelligence_os"]["layers"]["policy_decisions"]["requested_action"], "create_session_bundle"
        )
    ...
    def test_run_system_guard_chat_can_create_matchday_bundle(self, *_mocks):
        ...
        self.assertEqual(
            response["intelligence_os"]["layers"]["policy_decisions"]["requested_action"], "create_matchday_bundle"
        )
    ...
    def test_run_system_guard_chat_can_update_session(self, *_mocks):
        ...
        self.assertEqual(
            response["intelligence_os"]["layers"]["policy_decisions"]["requested_action"], "update_session"
        )
    ...
    def test_run_system_guard_chat_can_update_convocation(self, *_mocks):
        ...
        match = Match.objects.create(
            ...
            date=date(2026, 7, 6),
            home_team=self.team,
            away_team=Team.objects.create(name="Rival Edit", slug="rival-edit", group=group),
        )
        record = ConvocationRecord.objects.create(
            team=self.team, match=match, opponent_name="Rival Edit", match_date=date(2026, 7, 6), is_current=True
        )
        ...
        self.assertEqual(response["assistant_action"]["convocation"]["goalkeeper_id"], p1.id)
        self.assertEqual(
            response["intelligence_os"]["layers"]["policy_decisions"]["requested_action"], "update_convocation"
        )
    ...
    @patch(
        "football.system_guard.run_proactive_guard_cycle",
        return_value={"queue_counts": {"completed": 1}, "detections": [{"id": "demo"}]},
    )
    def test_maybe_run_scheduled_guard_cycle_respects_interval(self, mock_cycle):
        first = system_guard._maybe_run_scheduled_guard_cycle(
            workspace=self.workspace, actor_id=self.user.id, force=False
        )
        second = system_guard._maybe_run_scheduled_guard_cycle(
            workspace=self.workspace, actor_id=self.user.id, force=False
        )
    ...
    @patch(
        "football.system_guard._maybe_run_scheduled_guard_cycle",
        return_value={"ran": False, "reason": "interval_not_elapsed"},
    )
    ...
    def test_system_guard_page_renders_for_workspace_manager(
        self, _is_admin, _can_manage, mock_team, mock_workspace, _mock_cycle
    ):
    ...
    @patch("football.views._can_manage_workspace", return_value=True)
    @patch("football.views._is_admin_user", return_value=False)
    def test_system_guard_chat_api_returns_observability(
        self, _is_admin, _can_manage, _can_access, mock_team, mock_workspace, _mock_cycle
    ):
        mock_workspace.return_value = self.workspace
        mock_team.return_value = self.team
        system_guard._store_pref_value(
    ...
    @patch(
        "football.system_guard.run_system_guard_chat",
        return_value={
            "report": {"issue_summary": {"blockers": 0, "warnings": 0}},
            "chat": {
                "degraded": False,
                "response": {
                    ...
                    "highlights": [],
                    "actions": [],
                    "metrics": {"executed_tools": 0, "latency_ms": 0},
                },
            },
        },
    )
    ...
    @patch("football.views._is_admin_user", return_value=False)
    def test_system_guard_chat_api_allows_non_manager_user_but_sanitizes_execution(
        self, _is_admin, _can_manage, _can_access, mock_team, mock_workspace, mock_guard, _mock_ob
    ):
        mock_workspace.return_value = self.workspace
        mock_team.return_value = self.team
        self.client.force_login(self.user)
    ...
    @patch("football.views._can_manage_workspace", return_value=True)
    @patch("football.views._is_admin_user", return_value=False)
    def test_system_guard_chat_api_passes_sanitized_ui_snapshot(
        self, _is_admin, _can_manage, _can_access, mock_team, mock_workspace, mock_guard, _mock_ob
    ):
        mock_workspace.return_value = self.workspace
        mock_team.return_value = self.team
            ...
            data=json.dumps(
                {
                    ...
                    "page_context": {
                        ...
                        "health_snapshot": {
                            ...
                            "degraded_modules": [
                                {"label": "Tactica 3D", "notice_count": 1, "media_count": 1, "text_density": 42}
                            ],
                            "blocked_modules": [
                                {"label": "Resumen rival", "action_count": 0, "form_count": 0, "text_density": 4}
                            ],
        ...
        self.assertEqual(
            kwargs["page_context"]["visual_snapshot"]["render_alerts"][0],
            "Superficie Representacion 3D de tarea: escena 3D abierta sin objetos",
        )
        ...
        self.assertEqual(
            kwargs["page_context"]["runtime_snapshot"]["js_errors"][0]["message"],
            "ReferenceError: tacticScene is not defined",
        )
    ...
    def test_route_health_snapshot_marks_tactics_route_blocked_when_runtime_and_modules_fail(self):
        snapshot = system_guard._route_health_snapshot(
            page_context={
                "page": "coach-tactics",
                "path": "/coach/tactics/",
                "module_snapshot": {
    ...
    def test_browser_visual_openai_analysis_sends_real_image_input(self, mock_post):
                    ...
                    "output_text": json.dumps(
                        {
                            ...
                            "button_visibility": {
                                "state": "visible",
                                "detail": "Los controles principales se aprecian.",
                            },
        ...
        with patch.dict(
            ...
            result = system_guard._browser_visual_openai_analysis(
                page_context={
    ...
    def test_ollana_assessment_snapshot_reports_low_contrast_title(self):
        snapshot = system_guard._ollana_assessment_snapshot(
            page_context={
                "browser_visual_snapshot": {
                    "enabled": True,
                    "title": "Ficha de tarea",
    ...
    @patch(
        "football.system_guard._browser_visual_openai_analysis", return_value={"enabled": False, "reason": "disabled"}
    )
    @patch(
        "football.system_guard._browser_visual_ollama_analysis",
        return_value={"enabled": True, "provider": "ollama", "model": "llama3.2-vision"},
    )
    def test_browser_visual_ai_analysis_uses_local_fallback(self, mock_ollama, mock_openai):
        result = system_guard._browser_visual_ai_analysis(
            page_context={
                "browser_visual_snapshot": {
                    "enabled": True,
        ...
        class FakePlaywright:
            def __init__(self):
                ...
                self.webkit = type(
                    ...
                    {
                        "name": "webkit",
                        "launch": lambda self, **kwargs: (_ for _ in ()).throw(RuntimeError("webkit_failed")),
                    },
                )()
                self.stopped = False
    ...
    def test_operator_web_research_snapshot_uses_public_urls(self, mock_compact, mock_fetch):
        mock_fetch.return_value = [
            {"url": "https://example.com/", "ok": True, "method": "browser", "title": "Example", "text": "Contenido"}
        ]
        mock_compact.return_value = [
            {"url": "https://example.com/", "ok": True, "method": "browser", "title": "Example", "text": "Contenido"}
        ]
        snapshot = system_guard._operator_web_research_snapshot(page_context={"web_urls": "https://example.com"})
        self.assertTrue(snapshot["enabled"])
    ...
    def test_operator_web_research_snapshot_uses_search_query_when_no_urls(self, mock_compact, mock_fetch, mock_search):
        mock_search.return_value = [
            {"url": "https://example.com/doc", "ok": True, "method": "search", "title": "Doc", "snippet": "Snippet"}
        ]
        mock_fetch.return_value = [
            {"url": "https://example.com/doc", "ok": True, "method": "browser", "title": "Doc", "text": "Contenido"}
        ]
        mock_compact.return_value = [
            {"url": "https://example.com/doc", "ok": True, "method": "browser", "title": "Doc", "text": "Contenido"}
        ]
        snapshot = system_guard._operator_web_research_snapshot(page_context={"web_search_query": "example query"})
        self.assertTrue(snapshot["enabled"])
        self.assertEqual(snapshot["reason"], "fetched")
        self.assertEqual(snapshot["ok_count"], 1)
    ...
    @patch("football.views._can_manage_workspace", return_value=True)
    @patch("football.views._is_admin_user", return_value=False)
    def test_system_guard_chat_api_downgrades_code_ops_for_non_operator_manager(
        self, _is_admin, _can_manage, _can_access, mock_team, mock_workspace, mock_guard, _mock_ob
    ):
        mock_workspace.return_value = self.workspace
        mock_team.return_value = self.team
    ...
    @patch("football.views._can_manage_workspace", return_value=True)
    @patch("football.views._is_admin_user", return_value=False)
    def test_system_guard_chat_api_allows_whitelisted_operator_user(
        self, _is_admin, _can_manage, _can_access, mock_team, mock_workspace, mock_guard, _mock_ob
    ):
        mock_workspace.return_value = self.workspace
        mock_team.return_value = self.team
    ...
    def test_system_guard_chat_api_marks_admin_total_operator_context(
        self, _is_admin, _can_manage, _can_access, mock_team, mock_workspace, mock_guard, _mock_ob
    ):
        mock_workspace.return_value = self.workspace
        mock_team.return_value = self.team
    ...
    def test_operator_role_context_includes_cybersecurity_senior_knowledge(self):
        role_context = system_guard._operator_role_context(
            page_context={
                "page": "dashboard-home",
                "workspace_id": self.workspace.id,
        ...
        self.assertIn("cybersecurity_senior", role_context["active_roles"])
        self.assertIn("threat_modeling", role_context["knowledge_domains"])
    ...
    def test_system_brain_snapshot_exposes_cybersecurity_senior_role(self):
        brain = system_guard._system_brain_snapshot(
            self.workspace,
            page_context={"page": "dashboard-home", "workspace_id": self.workspace.id, "team_id": self.team.id},
            operator_profile={},
    ...
    @patch(
        ...
        return_value={
            ...
            "chat": {
                "degraded": True,
                "response": {"status": "ok", "message": "fallback", "highlights": [], "actions": []},
            },
        },
    )
    def test_system_guard_chat_api_returns_new_contract(
        self, mock_guard, _is_admin, _can_manage, mock_team, mock_workspace
    ):
        mock_workspace.return_value = self.workspace
        mock_team.return_value = self.team
        self.client.force_login(self.user)
...
class PendingCardsRivalReportTests(TestCase):
    ...
    def test_pending_card_appears_when_no_reports_exist(self):
        cards = dashboard_pending_services.build_team_pending_cards(
            self.primary_team, weekly_brief=self._brief_for("CD Rival")
        )
    ...
    def test_pending_card_disappears_when_pptx_export_exists(self):
        ...
        report.pptx_file.save(
            "informe-cd-rival.pptx", SimpleUploadedFile("informe-cd-rival.pptx", b"pptx-bytes"), save=True
        )
        cards = dashboard_pending_services.build_team_pending_cards(
            self.primary_team, weekly_brief=self._brief_for("CD Rival")
        )
...
class ManualOverridesAndKpiConsistencyTests(TestCase):
    def test_manual_base_overrides_survive_manual_match_for_other_player(self):
        ...
        PlayerStatistic.objects.create(
            player=player_a, season=season, match=None, context="manual-base", name="manual_pj", value=31
        )
        PlayerStatistic.objects.create(
            player=player_a, season=season, match=None, context="manual-base", name="manual_pt", value=29
        )
        PlayerStatistic.objects.create(
            player=player_a, season=season, match=None, context="manual-base", name="manual_minutes", value=2465
        )
        PlayerStatistic.objects.create(
            player=player_b, season=season, match=match, context="manual-match", name="manual_minutes", value=45
        )
    ...
    def test_manual_match_stats_override_stale_base_for_same_player(self):
        ...
        PlayerStatistic.objects.create(
            player=player, season=season, match=None, context="manual-base", name="manual_pj", value=5
        )
        PlayerStatistic.objects.create(
            player=player, season=season, match=None, context="manual-base", name="manual_pt", value=5
        )
        PlayerStatistic.objects.create(
            player=player, season=season, match=None, context="manual-base", name="manual_minutes", value=230
        )
        PlayerStatistic.objects.create(
            player=player, season=season, match=None, context="manual-base", name="manual_goals", value=6
        )
        PlayerStatistic.objects.create(
            player=player, season=season, match=None, context="manual-base", name="manual_assists", value=12
        )
    ...
    def test_participation_uses_full_season_minutes(self, _mock_rounds):
        ...
        PlayerStatistic.objects.create(
            player=player, season=season, match=None, context="manual-base", name="manual_minutes", value=450
        )
...
class MatchdayQuickButtonsTests(TestCase):
    def setUp(self):
        ...
        WorkspaceMembership.objects.create(
            workspace=self.workspace, user=self.member, role=WorkspaceMembership.ROLE_MEMBER
        )
        WorkspaceTeam.objects.create(workspace=self.workspace, team=self.team, is_default=True)
        ...
        self.conv = ConvocationRecord.objects.create(
            team=self.team, match=self.match, opponent_name="Rival QB", is_current=True
        )
...
class CriticalPagesSmokeTests(TestCase):
    def setUp(self):
        ...
        rival = Team.objects.create(
            name="Rival Smoke", slug="rival-smoke", short_name="RIV", group=group, is_primary=False
        )
        self.match = Match.objects.create(
            season=season,
            group=group,
        ...
        conv = ConvocationRecord.objects.create(
            team=self.team, match=self.match, opponent_name="Rival", is_current=True
        )
        ...
        self.video = RivalVideo.objects.create(
            team=self.team, rival_team=rival, title="Video Smoke", video=video_file, source=RivalVideo.SOURCE_MANUAL
        )
    ...
    def test_team_agenda_create_session_convoke_players(self):
        ...
        self.assertIn(response.status_code, {301, 302})
        session_obj = (
            TrainingSession.objects.filter(microcycle__team=self.team, focus="Entreno agenda").order_by("-id").first()
        )
...
class SearchApiExtendedGroupsTests(TestCase):
    ...
    def test_search_api_prioritizes_exact_team_matches(self):
        ...
        self.assertEqual(
            items[0].get("url"), reverse("platform-workspace-team-detail", args=[self.workspace.id, self.team_b.id])
        )
...
class SeasonHistoryServicesTests(TestCase):
    ...
    def test_workspace_season_keeps_team_and_player_history_when_player_deactivates(self):
        ...
        workspace_player = WorkspacePlayer.objects.get(
            workspace=self.workspace, player=self.player, current_team=self.team
        )
    ...
    def test_roster_create_player_saves_extended_profile_fields(self):
        ...
        self.assertTrue(
            WorkspaceSeasonPlayer.objects.filter(season=self.season, player=player, team=self.team).exists()
        )

    def test_roster_move_player_to_next_category_keeps_him_active_in_club(self):
        target_team = Team.objects.create(
            name="Equipo Destino", slug="equipo-destino", short_name="Destino", category="Cadete"
        )
    ...
    def test_roster_stats_cards_do_not_reuse_previous_season_manual_base_stats(self):
        competition = Competition.objects.create(
            name="Liga Cards Antigua", slug="liga-cards-antigua", level=1, region="test"
        )
        ...
        PlayerStatistic.objects.create(
            player=self.player, season=federation_season, match=None, context="manual-base", name="manual_pj", value=31
        )
        PlayerStatistic.objects.create(
            player=self.player,
            season=federation_season,
            match=None,
            context="manual-base",
            name="manual_minutes",
            value=2465,
    ...
    def test_static_player_photo_fallback_follows_player_after_category_change(self):
        ...
        youth_team = Team.objects.create(
            name="Benjamín A", slug="benjamin-a", short_name="Benjamin A", category="Benjamín"
        )
    ...
    def test_static_player_photo_fallback_does_not_match_only_by_number(self):
        ...
        youth_team = Team.objects.create(
            name="Benagalbon Benjamin", slug="benagalbon-benjamin", short_name="Benjamin", category="Benjamín"
        )
...
class TrialPaywallTests(TestCase):
    def test_trial_expired_redirects_to_billing_for_html_pages(self):
        ...
        self.client.force_login(user)
        response = self.client.get(
            f"{reverse('coach-roster')}?workspace={workspace.id}", secure=True, HTTP_ACCEPT="text/html"
        )
        self.assertIn(response.status_code, {301, 302})
...
class PlayerUserLinkTests(TestCase):
    def test_resolve_player_uses_explicit_user_link(self):
        ...
        u1 = get_user_model().objects.create_user(
            username="angel.ayala", password="pass-1234", first_name="Angel", last_name="Ayala"
        )
        u2 = get_user_model().objects.create_user(
            username="angel.sanchez", password="pass-1234", first_name="Angel", last_name="Sanchez"
        )
...
class TaskStudioAccessTests(TestCase):
    def setUp(self):
        ...
        self.own_task = TaskStudioTask.objects.create(
            owner=self.user, title="Tarea propia", block=SessionTask.BLOCK_MAIN_1, duration_minutes=15
        )
        self.other_task = TaskStudioTask.objects.create(
            owner=self.other_user, title="Tarea ajena", block=SessionTask.BLOCK_MAIN_1, duration_minutes=15
        )
    ...
    def test_task_studio_admin_can_delete_foreign_task(self):
        ...
        response = self.client.post(
            reverse("task-studio-task-delete", args=[self.other_task.id]) + f"?user={self.other_user.id}"
        )
...
class PlatformWorkspaceTests(TestCase):
    def setUp(self):
        ...
        self.team = Team.objects.create(
            name="Benagalbón matriz", slug="benagalbon-matriz", group=group, is_primary=True
        )
        self.alt_team = Team.objects.create(
            name="Cliente alternativo", slug="cliente-alternativo", group=group, is_primary=False
        )
    ...
    def test_platform_overview_bootstraps_primary_and_task_studio_workspaces(self):
        ...
        studio_workspace = Workspace.objects.filter(
            owner_user=self.studio_user, kind=Workspace.KIND_TASK_STUDIO
        ).first()
    ...
    def test_platform_overview_documents_tab_shows_recent_documents(self):
        ...
        response = self.client.get(reverse("platform-overview"))
        studio_workspace = Workspace.objects.filter(
            kind=Workspace.KIND_TASK_STUDIO, owner_user=self.studio_user
        ).first()
        microcycle = TrainingMicrocycle.objects.create(
            team=self.team,
            title="MD-1",
    ...
    def test_platform_overview_can_generate_global_invitation(self):
        ...
        self.assertEqual(response.status_code, 200)
        invitation = (
            UserInvitation.objects.filter(user=self.studio_user, is_active=True).order_by("-created_at").first()
        )
    ...
    def test_dashboard_data_uses_active_club_season_for_home_metrics(self):
        ...
        player_card = next(
            item for item in payload["player_cards"] if int(item.get("player_id") or 0) == int(player.id)
        )
    ...
    def test_dashboard_data_ignores_undated_current_convocation_for_next_match(self):
        future_rival = Team.objects.create(
            name="Rival Futuro Dashboard", slug="rival-futuro-dashboard", group=self.alt_team.group
        )
        Match.objects.create(
            season=self.alt_team.group.season,
            group=self.alt_team.group,
    ...
    def test_workspace_detail_can_invite_member(self):
        ...
        invitation = (
            UserInvitation.objects.filter(user__username="invite-demo", is_active=True).order_by("-created_at").first()
        )
        self.assertIsNotNone(invitation)
    ...
    def test_platform_overview_does_not_recreate_deleted_task_studio_workspace(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertFalse(
            Workspace.objects.filter(kind=Workspace.KIND_TASK_STUDIO, owner_user=self.studio_user).exists()
        )
...
class ManualStatsTests(TestCase):
    ...
    def test_saved_manual_base_totals_override_manual_match_rows(self):
        ...
        opponent = Team.objects.create(
            name="Rival Manual", slug="rival-manual", group=self.team.group, is_primary=False
        )
        match = Match.objects.create(
            season=self.season,
            group=self.team.group,
        ...
        PlayerStatistic.objects.create(
            player=self.player, season=self.season, match=match, context="manual-match", name="manual_minutes", value=45
        )
        PlayerStatistic.objects.create(
            player=self.player, season=self.season, match=match, context="manual-match", name="manual_goals", value=1
        )
        PlayerStatistic.objects.create(
            player=self.player, season=self.season, match=match, context="manual-match", name="manual_assists", value=0
        )
...
class AdminActionsTests(TestCase):
    ...
    def test_coach_matches_clean_get_ignores_saved_registered_filter(self):
        self.client.force_login(self.user)
        unregistered_rival = Team.objects.create(
            name="Rival Sin Registro", slug="rival-sin-registro", group=self.team.group
        )
        Match.objects.create(
...
class KpiAuditEndpointTests(TestCase):
    def setUp(self):
        ...
        self.match = Match.objects.create(
            season=season, group=group, home_team=self.team, away_team=self.rival, round="1"
        )
...
class MatchActionWorkflowTests(TestCase):
    def setUp(self):
        ...
        self.team = Team.objects.create(
            name="Benagalbón Acciones", slug="benagalbon-acciones", group=group, is_primary=True
        )
...
class PlayerDashboardViewTests(TestCase):
    ...
    def test_player_dashboard_uses_active_club_season_dates(self):
        ...
        self.assertEqual(response.status_code, 200)
        player_row = next(
            row for row in response.context["player_stats"] if int(row.get("player_id") or 0) == int(self.player.id)
        )
...
class CoachTrainerMetricsTests(TestCase):
    ...
    def test_trainer_overview_counts_assist_as_completed_pass(self):
        ...
        self.assertEqual(response.status_code, 200)
        overview_summary = {
            item["label"]: item["value"] for item in response.context["coach_overview_stats"]["summary"]
        }
    ...
    def test_trainer_heatmap_infers_missing_zone_from_match_profile(self):
        ...
        self.assertEqual(response.status_code, 200)
        medio_centro = next(
            zone for zone in response.context["coach_total_field_zones"] if zone["key"] == "Medio Centro"
        )
...
class StaffUserLinkingTests(TestCase):
    def setUp(self):
        ...
        WorkspaceMembership.objects.create(
            workspace=self.workspace, user=self.admin, role=WorkspaceMembership.ROLE_ADMIN
        )
    ...
    def test_create_tab_redirects_to_dedicated_editor_flow(self):
        ...
        self.assertContains(response, "Pizarra + ficha")
        self.assertNotContains(response, "Crear tarea con pizarra")

    def test_task_create_page_redirects_to_real_task_sheet(self):
        response = self.client.get(reverse("sessions-task-create"))

        self.assertEqual(response.status_code, 302)
        self.assertIn("/coach/sesiones/tarea/", response["Location"])
        self.assertIn("tab=presentation", response["Location"])
        self.assertIn("task_flow=create", response["Location"])
        self.assertIn("format=club", response["Location"])
        self.assertEqual(SessionTask.objects.filter(title="Nueva tarea").count(), 1)

    def test_task_builder_uses_distinct_pitch3d_trigger_in_standard_mode(self):
        session = TrainingSession.objects.create(
            microcycle=self.microcycle,
            session_date=date(2026, 3, 25),
            focus="Sesión builder",
            duration_minutes=90,
        )
        task = SessionTask.objects.create(
            session=session,
        ...
        response = self.client.get(f"{reverse('sessions-task-edit', args=[task.id])}?embedded=1")
    ...
    def test_task_builder_route_redirects_to_task_sheet_when_not_embedded(self):
        session = TrainingSession.objects.create(
            microcycle=self.microcycle,
            session_date=date(2026, 3, 25),
            focus="Sesión redirección",
            duration_minutes=90,
        )
        task = SessionTask.objects.create(
            session=session,
            title="Tarea redirección",
            block=SessionTask.BLOCK_MAIN_1,
            duration_minutes=15,
            tactical_layout={"meta": {"scope": "coach"}},
        )
        ...
        self.assertEqual(response.status_code, 302)
        self.assertIn(reverse("session-task-detail", args=[task.id]), response["Location"])
        self.assertIn("tab=edit", response["Location"])
        self.assertIn("edit_tab=graphic", response["Location"])
    ...
    def test_task_builder_shows_roster_bank_with_visual_player_cards(self):
        ...
        session = TrainingSession.objects.create(
            microcycle=self.microcycle,
            session_date=date(2026, 3, 25),
            focus="Sesión banco visual",
            duration_minutes=90,
        )
        task = SessionTask.objects.create(
            session=session,
            title="Tarea banco visual",
            block=SessionTask.BLOCK_MAIN_1,
            duration_minutes=15,
            tactical_layout={"meta": {"scope": "coach"}},
        )
        ...
        self.assertContains(response, "resource-tab-bar")

    def test_task_builder_shows_grass_style_selector(self):
        session = TrainingSession.objects.create(
            microcycle=self.microcycle,
            session_date=date(2026, 3, 25),
            focus="Sesión césped",
        ...
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, 'id="pitch-grass-select"')
        self.assertContains(response, 'option value="broadcast_premium"')

    def test_task_builder_prefills_age_group_from_team_category(self):
        self.team.category = "Juvenil"
        self.team.save(update_fields=["category"])

        response = self.client.get(reverse("sessions-task-create"), follow=True)

        self.assertEqual(response.status_code, 200)
        self.assertContains(response, 'name="task_age_group" value="Juvenil"')

    def test_task_create_sheet_uses_inline_club_form_without_legacy_settings_panel(self):
        response = self.client.get(reverse("sessions-task-create"), follow=True)

        self.assertEqual(response.status_code, 200)
        self.assertContains(response, "Ficha Club")
        self.assertContains(response, ">Club<", html=False)
        self.assertContains(response, 'name="task_title"')
        self.assertContains(response, 'name="task_objective"')
        self.assertContains(response, 'name="task_structure"', count=1)
        self.assertContains(response, "data-inline-player-builder")
        self.assertContains(response, "data-inline-material-builder")
        self.assertContains(response, "Añadir pizarra")
        self.assertContains(response, "Ir a pizarra")
        self.assertNotContains(response, "Modo creación activo sobre ficha Club")
        self.assertNotContains(response, 'data-task-edit-tab="settings"')
        self.assertNotContains(response, "<h2>Configuración</h2>", html=True)

    def test_task_detail_update_persists_structured_players_and_materials(self):
        session = TrainingSession.objects.create(
            microcycle=self.microcycle,
            session_date=date(2026, 3, 25),
            focus="Sesión ficha inline",
            duration_minutes=90,
        )
        task = SessionTask.objects.create(
            session=session,
        ...
        response = self.client.post(
            reverse("session-task-detail", args=[task.id]),
            {
                ...
                "task_title": "Tarea inline",
                "task_block": SessionTask.BLOCK_MAIN_1,
                "task_minutes": "15",
                "task_player_group_role": ["equipo_a", "equipo_b", "porteros"],
                "task_player_group_qty": ["6", "6", "2"],
                "task_material_group_kind": ["cone", "bib", "goal"],
                "task_material_group_qty": ["12", "8", "2"],
                "task_graphic_panel_type": ["2d_classic", "2d_interactive", "3d_interactive"],
            },
        )
        ...
        sheet = analysis.get("task_sheet") or {}

        self.assertEqual(meta.get("player_count"), "6 Equipo A, 6 Equipo B, 2 Porteros")
        self.assertEqual(meta.get("player_total"), 14)
        self.assertEqual(sheet.get("players"), "6 Equipo A, 6 Equipo B, 2 Porteros")
        self.assertEqual(sheet.get("materials"), "12 Conos, 8 Petos, 2 Porterías")
        self.assertEqual((meta.get("player_groups") or [])[0].get("role"), "equipo_a")
        self.assertEqual((sheet.get("materials_groups") or [])[0].get("kind"), "cone")
        self.assertEqual(
            [item.get("type") for item in (meta.get("graphic_panels") or [])],
            ["2d_classic", "2d_interactive", "3d_interactive"],
        )

    def test_task_detail_redirects_to_graphic_editor_and_prepares_2d_3d_panels(self):
        session = TrainingSession.objects.create(
            microcycle=self.microcycle,
            session_date=date(2026, 3, 26),
            focus="Sesión lanzamiento gráfico",
            duration_minutes=90,
        )
        task = SessionTask.objects.create(
            session=session,
            title="Tarea lanzamiento",
            block=SessionTask.BLOCK_MAIN_1,
            duration_minutes=15,
            tactical_layout={"meta": {"scope": "coach"}},
        )
        ...
        response = self.client.post(
            f"{reverse('session-task-detail', args=[task.id])}?task_flow=create&format=club&tab=presentation",
            {
                "detail_action": "update_task_detail",
                "task_title": "Tarea lanzamiento",
                "task_block": SessionTask.BLOCK_MAIN_1,
                "task_minutes": "15",
                "task_graphic_launch_mode": "3d",
                "open_graphic_after_save": "1",
            },
        )
        ...
        self.assertIn("tab=edit", response["Location"])
        self.assertIn("edit_tab=graphic", response["Location"])
        self.assertIn("graphic_launch_mode=3d", response["Location"])
        task.refresh_from_db()
        meta = (task.tactical_layout or {}).get("meta") or {}
        self.assertEqual(
            [item.get("type") for item in (meta.get("graphic_panels") or [])], ["2d_classic", "3d_classic"]
        )

    def test_task_detail_create_graphic_tab_passes_3d_launch_mode_to_embedded_builder(self):
        session = TrainingSession.objects.create(
            microcycle=self.microcycle,
            session_date=date(2026, 3, 26),
            focus="Sesión embebida 3D",
            duration_minutes=90,
        )
        task = SessionTask.objects.create(
            session=session,
            title="Tarea embebida 3D",
            block=SessionTask.BLOCK_MAIN_1,
            duration_minutes=15,
            tactical_layout={"meta": {"scope": "coach"}},
        )
        ...
        response = self.client.get(
            reverse("session-task-detail", args=[task.id]),
            {
                ...
                "edit_tab": "graphic",
                "format": "club",
                "graphic_launch_mode": "3d",
            },
        )

        self.assertEqual(response.status_code, 200)
        self.assertContains(response, "graphic_launch_mode=3d")
        self.assertContains(response, "embedded=1")

    def test_task_pdf_3d_embed_exposes_full_stadium_payload(self):
        session = TrainingSession.objects.create(
            microcycle=self.microcycle,
            session_date=date(2026, 3, 26),
            focus="Sesión payload 3D",
            duration_minutes=90,
        )
        task = SessionTask.objects.create(
            session=session,
            title="Tarea payload 3D",
            block=SessionTask.BLOCK_MAIN_1,
            duration_minutes=15,
            tactical_layout={"meta": {"scope": "coach"}},
        )

        response = self.client.get(reverse("session-task-pdf-3d-embed", args=[task.id]))

        self.assertEqual(response.status_code, 200)
        self.assertContains(response, "stadiumModelUrl")
        self.assertContains(response, "stadiumTopHSrc")
        self.assertContains(response, "stadiumTopVSrc")
        self.assertContains(response, "stadiumOverlayHSrc")
        self.assertContains(response, "stadiumOverlayVSrc")

    @patch("football.views._build_task_pdf_context")
    def test_task_detail_create_presentation_disables_heavy_3d_snapshot(self, build_pdf_context):
        session = TrainingSession.objects.create(
            microcycle=self.microcycle,
            session_date=date(2026, 3, 26),
            focus="Sesión ficha rápida",
            duration_minutes=90,
        )
        task = SessionTask.objects.create(
            session=session,
            title="Tarea rápida 3D",
            block=SessionTask.BLOCK_MAIN_1,
            duration_minutes=15,
            tactical_layout={
                "meta": {
                    "scope": "coach",
                    "graphic_panels": [{"type": "2d_classic"}, {"type": "3d_classic"}],
                    "graphic_editor": {
                        "canvas_state": {
                            "version": "5.3.0",
                            "objects": [{"type": "circle", "left": 120, "top": 140, "radius": 12}],
                        },
                        "canvas_width": 1280,
                        "canvas_height": 720,
                    },
        ...
        build_pdf_context.return_value = {
            "graphic_panel_catalog": [],
            "graphic_panels": [],
            "graphic_view_2d_url": "",
            "graphic_view_3d_url": "",
            "graphic_view_3d_embed_url": "",
            "task_preview_3d_embed_url": "",
        }

        response = self.client.get(
            reverse("session-task-detail", args=[task.id]),
            {"task_flow": "create", "tab": "presentation", "format": "club"},
        )

        self.assertEqual(response.status_code, 200)
        self.assertTrue(build_pdf_context.called)
        render_bundle_options = build_pdf_context.call_args.kwargs.get("render_bundle_options") or {}
        self.assertFalse(render_bundle_options.get("include_3d_snapshot"))
        self.assertTrue(render_bundle_options.get("force_3d_embed_url"))

    def test_task_graphic_panel_catalog_uses_interactive_3d_when_classic_snapshot_missing(self):
        catalog = football_views._build_task_graphic_panel_catalog(
            {
                "graphic_view_2d_url": "data:image/png;base64,2d",
                "graphic_view_3d_url": "",
                "graphic_view_3d_embed_url": "/coach/sesiones/tarea/373/pdf-3d-embed/?camera=coach",
            }
        )

        panel = next(item for item in catalog if item.get("type") == "3d_classic")
        self.assertEqual(panel.get("render_mode"), "iframe")
        self.assertEqual(panel.get("src"), "/coach/sesiones/tarea/373/pdf-3d-embed/?camera=coach")
        self.assertTrue(panel.get("available"))

    def test_task_create_page_post_redirects_to_presentation_sheet(self):
        response = self.client.post(
            reverse("sessions-task-create"),
            {
                "task_title": "Tarea ficha",
                "task_objective": "Objetivo ficha",
                "task_block": SessionTask.BLOCK_MAIN_1,
                "task_minutes": "15",
            },
        )
        ...
        self.assertIn("tab=presentation", response["Location"])
        self.assertIn("format=club", response["Location"])
        self.assertEqual(SessionTask.objects.filter(title="Tarea ficha").count(), 1)

    def test_task_create_page_post_redirects_to_graphic_editor_when_requested(self):
        response = self.client.post(
            reverse("sessions-task-create"),
            {
                "task_title": "Tarea gráfica",
                "task_objective": "Objetivo gráfico",
                "task_block": SessionTask.BLOCK_MAIN_1,
                "task_minutes": "15",
                "create_open_graphic": "1",
            },
        )
        ...
        self.assertIn("tab=edit", response["Location"])
        self.assertIn("edit_tab=graphic", response["Location"])
        self.assertEqual(SessionTask.objects.filter(title="Tarea gráfica").count(), 1)
    ...
    def test_task_builder_rehydrates_performed_task_from_origin_graphic_state(self):
        ...
        response = self.client.get(f"{reverse('sessions-task-edit', args=[performed.id])}?embedded=1")
                ...
                "timeline": [
                    {
                        ...
                        "canvas_state": json.dumps(
                            {"version": "5.3.0", "objects": [{"type": "circle", "left": 12, "top": 13}]},
                            ensure_ascii=False,
                        ),
                    }
                ],
    ...
    def test_session_task_detail_is_default_view_for_editable_tasks(self):
        ...
        self.assertContains(response, "Pizarra gráfica")
        self.assertContains(response, "La ficha se completa arriba. Este es el único paso externo de edición.")
        ...
        self.assertContains(edit_response, "Pizarra gráfica")
        self.assertNotContains(edit_response, 'data-task-edit-tab="settings"')
        self.assertNotContains(edit_response, "<h2>Configuración</h2>", html=True)
    ...
    def test_session_task_detail_respects_selected_format(self):
        ...
        self.assertContains(uefa_response, "Resumen del ejercicio")
        self.assertContains(uefa_response, "Indicaciones del entrenador")
        self.assertContains(uefa_response, "Guia de intervencion")
        self.assertContains(uefa_response, "Lectura rapida")
        ...
        mock_call_ollama_json.return_value = (
            {
                ...
                "phase_tags": ["ataque"],
                "detected_materials": [
                    {"label": "CONO", "title": "Cono alto", "kind": "cone", "category": "delimitacion"}
                ],
                "quality_score": 92,
                "task_sheet": {
    ...
    def test_task_builder_embedded_save_and_back_redirects_to_detail_presentation(self):
        session = TrainingSession.objects.create(
            microcycle=self.microcycle,
            session_date=date(2026, 3, 25),
            focus="Sesión embebida",
            duration_minutes=90,
        )
        task = SessionTask.objects.create(
            session=session,
            title="Tarea embebida",
            block=SessionTask.BLOCK_MAIN_1,
            duration_minutes=15,
            tactical_layout={"meta": {"scope": "coach"}},
        )
        ...
        response = self.client.post(
            f"{reverse('sessions-task-edit', args=[task.id])}?back_to=detail&return_format=club&return_task_flow=create",
            {
                "planner_action": "create_draw_task",
                "draw_target_session_id": session.id,
                "draw_task_template": "none",
                "draw_task_title": "Tarea embebida",
                "draw_task_block": SessionTask.BLOCK_MAIN_1,
                "draw_task_minutes": "15",
                "draw_task_pitch_preset": "full_pitch",
                "draw_task_pitch_grass_style": "stadium_native",
                "draw_canvas_state": json.dumps({"version": "5.3.0", "objects": []}),
                "draw_canvas_width": "1280",
                "draw_canvas_height": "720",
                "save_and_back_to_detail": "1",
            },
        )
    ...
    def test_task_builder_preserves_existing_graphic_panels_when_saving_canvas_only(self):
        session = TrainingSession.objects.create(
            microcycle=self.microcycle,
            session_date=date(2026, 3, 25),
            focus="Sesión paneles",
            duration_minutes=90,
        )
        task = SessionTask.objects.create(
            session=session,
            ...
            tactical_layout={
                "meta": {
                    ...
                    "graphic_panels": [{"type": "2d_classic"}, {"type": "3d_classic"}],
                    "graphic_editor": {
                        "canvas_state": {"version": "5.3.0", "objects": [{"type": "circle", "left": 10, "top": 20}]},
                        "canvas_width": 1054,
                        "canvas_height": 684,
                    },
                },
        ...
        response = self.client.post(
            reverse("sessions-task-edit", args=[task.id]),
            {
                ...
                "draw_target_session_id": session.id,
                "draw_task_template": "none",
                "draw_task_title": "Tarea paneles",
                "draw_task_block": SessionTask.BLOCK_MAIN_1,
                "draw_task_minutes": "15",
                "draw_task_pitch_preset": "full_pitch",
                "draw_task_pitch_grass_style": "stadium_native",
                "draw_canvas_state": json.dumps(
                    {"version": "5.3.0", "objects": [{"type": "circle", "left": 40, "top": 50}]}
                ),
                "draw_canvas_width": "1280",
                "draw_canvas_height": "720",
            },
    ...
    def test_task_builder_invalidates_stale_preview_when_saving_canvas_without_preview_data(self):
        session = TrainingSession.objects.create(
            microcycle=self.microcycle,
            session_date=date(2026, 3, 25),
            focus="Sesión preview stale",
            duration_minutes=90,
        )
        task = SessionTask.objects.create(
            session=session,
            title="Tarea preview stale",
            block=SessionTask.BLOCK_MAIN_1,
            duration_minutes=15,
            tactical_layout={
                "meta": {
                    "scope": "coach",
                    "preview_data_embedded_v1": "data:image/png;base64,AAAA",
                    "graphic_editor": {
                        "canvas_state": {"version": "5.3.0", "objects": []},
                        "canvas_width": 1054,
                        "canvas_height": 684,
        ...
        task.task_preview_image.save("stale-preview.png", ContentFile(b"old-preview"), save=True)
        ...
        response = self.client.post(
            reverse("sessions-task-edit", args=[task.id]),
            {
                ...
                "draw_target_session_id": session.id,
                "draw_task_template": "none",
                "draw_task_title": "Tarea preview stale",
                "draw_task_block": SessionTask.BLOCK_MAIN_1,
                "draw_task_minutes": "15",
                "draw_task_pitch_preset": "full_pitch",
                "draw_task_pitch_grass_style": "stadium_native",
                "draw_canvas_state": json.dumps(
                    {"version": "5.3.0", "objects": [{"type": "circle", "left": 80, "top": 90}]}
                ),
                "draw_canvas_width": "1280",
                "draw_canvas_height": "720",
            },
        ...
        self.assertEqual(response.status_code, 200)
        task.refresh_from_db()
        self.assertFalse(bool(getattr(task, "task_preview_image", None)))
        meta = (task.tactical_layout or {}).get("meta") or {}
        self.assertEqual(str(meta.get("preview_data_embedded_v1") or ""), "")

    def test_task_detail_create_flow_renders_saved_2d_graphic_panel(self):
        session = TrainingSession.objects.create(
            microcycle=self.microcycle,
            session_date=date(2026, 3, 25),
            focus="Sesión ficha con pizarra",
            duration_minutes=90,
        )
        task = SessionTask.objects.create(
            session=session,
            title="Tarea con pizarra",
            block=SessionTask.BLOCK_MAIN_1,
            duration_minutes=15,
            tactical_layout={
                "meta": {
                    "scope": "coach",
                    "graphic_panels": [{"type": "2d_classic"}],
                    "preview_data_embedded_v1": "data:image/png;base64,AAAA",
                    "graphic_editor": {
                        "canvas_state": {"version": "5.3.0", "objects": [{"type": "circle", "left": 10, "top": 20}]},
                        "canvas_width": 1054,
        ...
        task.task_preview_image.save("task-preview.png", ContentFile(b"preview-bytes"), save=True)
        ...
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, "Pizarra 2D clásica")
        self.assertNotContains(response, "No hay representaciones gráficas disponibles todavía.")
    ...
    def test_session_task_pdf_preview_renders_without_saving_task(self):
        response = self.client.post(
            reverse("sessions-task-pdf-preview") + "?style=uefa",
            {
                ...
                "draw_canvas_state": json.dumps({"version": "5.3.0", "objects": []}),
                "draw_canvas_preview_data": "data:image/png;base64,"
                + base64.b64encode(b"preview-image").decode("ascii"),
            },
        )
                ...
                "timeline": [
                    {
                        ...
                        "canvas_state": {
                            "version": "5.3.0",
                            "objects": [{"type": "circle", "left": 120, "top": 130, "radius": 8}],
                        },
                    },
                    {
                        "title": "Llegada",
                        ...
                        "canvas_state": {
                            "version": "5.3.0",
                            "objects": [{"type": "circle", "left": 300, "top": 130, "radius": 8}],
                        },
                    },
                ],
...
class CoachOverviewTests(TestCase):
    ...
    @patch("football.views.load_universo_snapshot")
    @patch("football.views.load_preferred_next_match_payload", return_value=None)
    def test_coach_overview_keeps_team_row_in_compact_standings_when_outside_top_positions(
        self, _mock_next, mock_snapshot
    ):
        mock_snapshot.return_value = {
            "standings": [
...
class TaskBuilderUiVisibilityTests(TestCase):
    ...
    def test_task_builder_command_menu_is_hidden_by_default(self):
        ...
        """
        self.client.force_login(self.user)
        team = Team.objects.get(slug="equipo-pruebas")
        microcycle = TrainingMicrocycle.objects.create(
            team=team,
            title="Microciclo builder",
            week_start=date(2026, 3, 23),
            week_end=date(2026, 3, 29),
        )
        session = TrainingSession.objects.create(
...
class ClubOnboardingImportTests(TestCase):
    ...
    def test_universo_candidate_binds_group_without_live_token(self):
        ...
        workspace = Workspace.objects.create(
            name="Club", slug="club", kind=Workspace.KIND_CLUB, primary_team=team, owner_user=self.user
        )
        context = WorkspaceCompetitionContext.objects.create(
            workspace=workspace,
            team=team,
...
class SessionsPlannerTaskAssignTests(TestCase):
    ...
    def test_assign_task_shows_in_selected_block_after_post(self):
        ...
        self.assertEqual(response.status_code, 200)
        copied = (
            SessionTask.objects.filter(session=self.session, title="Tarea interactiva", deleted_at__isnull=True)
            .order_by("-id")
            .first()
        )
    ...
    def test_assign_task_works_when_context_only_in_post(self):
        ...
        self.assertEqual(response.status_code, 200)
        copied = SessionTask.objects.filter(session=self.session, deleted_at__isnull=True).order_by("-id").first()
        self.assertIsNotNone(copied)
...
class SessionsPlannerCreateSessionTests(TestCase):
    ...
    def test_create_session_plan_persists_when_team_and_workspace_are_posted(self):
        ...
        self.assertEqual(response.status_code, 200)
        created = TrainingSession.objects.filter(
            microcycle=self.microcycle, focus__iexact="Transición + finalización"
        ).first()
        self.assertIsNotNone(created)
    ...
    def test_create_microcycle_page_renders_methodology_fields(self):
        self.client.force_login(self.user)
        url = (
            f"{reverse('sessions')}?tab=create&create_page=microcycle&team={self.team.id}&workspace={self.workspace.id}"
        )
        response = self.client.get(url)
        self.assertEqual(response.status_code, 200)
        html = response.content.decode("utf-8", errors="ignore")
...
class SessionsPlannerLoadPastSessionTests(TestCase):
    ...
    def test_load_past_session_without_tab_param_stays_on_sessions_tab(self):
        self.client.force_login(self.user)
        url = (
            f"{reverse('sessions')}?workspace={self.workspace.id}&team={self.team.id}&session_id={self.past_session.id}"
        )
        response = self.client.get(url)
        self.assertEqual(response.status_code, 200)
        html = response.content.decode("utf-8", errors="ignore")
...
class AcademyEndpointTests(TestCase):
    def setUp(self):
        ...
        WorkspaceMembership.objects.create(
            workspace=self.workspace_enabled, user=self.user, role=WorkspaceMembership.ROLE_OWNER
        )
        ...
        WorkspaceMembership.objects.create(
            workspace=self.workspace_disabled, user=self.user, role=WorkspaceMembership.ROLE_OWNER
        )
...
class AiTrainerLibraryTests(TestCase):
    ...
    def test_ai_trainer_can_save_task_to_separate_repository(self):
        ...
        task = (
            SessionTask.objects.filter(
                session__microcycle__team=self.team, deleted_at__isnull=True, title__startswith="IA ·"
            )
            .order_by("-id")
            .first()
        )
