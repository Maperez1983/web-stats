...
from . import task_library_services, workspace_context
...
def club_season_wizard(request):
    ...
    def _club_group_workspaces(root_workspace):
        ...
        try:
            candidates = list(
                Workspace.objects.filter(kind=Workspace.KIND_CLUB)
        ...
        for candidate in candidates:
            ...
            split_parent_key = (
                _normalize_team_lookup_key(candidate_name.split("·", 1)[0].strip()) if "·" in candidate_name else ""
            )
    ...
    def _club_rollover_teams(root_workspace):
        ...
        try:
            links = (
                WorkspaceTeam.objects.filter(workspace_id__in=group_ids)
    ...
    def _player_ids_for_teams(team_list):
        ...
        if not team_ids:
            return []
        return list(
            Player.objects.filter(team_id__in=team_ids)
    ...
    if request.method == "POST":
        ...
        try:
            if action == "close_season":
                ...
                for grouped_workspace in workspace_group_members:
                    ...
                    if grouped_season and bool(getattr(grouped_season, "is_active", True)):
                        try:
                            grouped_closed = close_workspace_season(
                                grouped_workspace, season=grouped_season, end_date=end_date
                            )
            ...
            if action in {"questionnaire_save", "questionnaire_save_next", "questionnaire_finish"}:
                state = dict(state or {})
                season_id = int(_parse_int(state.get("new_season_id")) or 0) or int(
                    getattr(getattr(workspace, "active_season", None), "id", 0) or 0
                )
                if not season_id:
                ...
                membership = (
                    WorkspaceSeasonPlayer.objects.filter(id=int(membership_id), season_id=int(season_id))
                ...
                def _clean_text(field, *, max_len=800):
                    return task_library_services.sanitize_task_text(
                        (request.POST.get(field) or "").strip(), multiline=True, max_len=max_len
                    )
                ...
                membership.save(
                    update_fields=["questionnaire_v", "questionnaire", "questionnaire_completed_at", "updated_at"]
                )
            ...
            if action == "create_new_season":
                season_label = task_library_services.sanitize_task_text(
                    (request.POST.get("season_label") or "").strip(), multiline=False, max_len=32
                )
                ...
                for season_team in season_teams:
                    ...
                    try:
                        WorkspaceTeam.objects.get_or_create(
                            ...
                            team=season_team,
                            defaults={
                                "is_default": int(getattr(season_team, "id", 0) or 0)
                                == int(getattr(active_team, "id", 0) or 0)
                            },
                        )
                    except Exception:
                        pass
            ...
            log_exception(
                logger,
                "No se pudo completar el cierre de temporada.",
                request,
                workspace=workspace,
                team=active_team,
                action=action,
    ...
    try:
        q_season_id = int(_parse_int((state or {}).get("new_season_id")) or 0) or int(
            getattr(getattr(workspace, "active_season", None), "id", 0) or 0
        )
        if q_season_id:
        ...
        if q_season:
            q_memberships = list(
                WorkspaceSeasonPlayer.objects.filter(season=q_season)
                .select_related("player")
            ...
            if q_membership:
                q_rating_summary = build_questionnaire_rating_summary(
                    getattr(q_membership, "questionnaire", None) or {}
                )
    except Exception:
        q_season = None
        q_memberships = []
