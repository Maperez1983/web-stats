...
def serialize_standings(group):
    ...
    if sibling_group:
        sibling_is_better = current_meta["total"] == 0 or (
            sibling_group["group_id"] != group.id
            and sibling_group["latest"]
            and (not current_meta["latest"] or sibling_group["latest"] > current_meta["latest"])
        )
        ...
        normalized.append(
            {
                ...
                "crest_url": str(
                    row.get("crest_url") or crest_lookup.get(_normalize_team_lookup_key(team)) or ""
                ).strip(),
...
def resolve_standings_for_team(primary_team, snapshot=None, provider=None):
    ...
    if db_rows:
        return db_rows
    if bool(getattr(primary_team, "is_primary", False)) and universo_snapshot_supports_team(snapshot, primary_team):
        universo_rows = serialize_universo_standings(snapshot)
        if universo_rows:
            return universo_rows
    return serialize_standings(primary_team.group)
