...
from .team_media_services import absolute_universo_url, build_team_crest_lookup, sanitize_universo_external_image
...
def serialize_universo_live_classification(payload):
    ...
    for row in rows:
        if not isinstance(row, dict):
            continue
        team = str(row.get("nombre") or row.get("Nombre_equipo") or row.get("equipo") or row.get("team") or "").strip()
        if not team:
            continue
        team_code = str(row.get("codequipo") or row.get("CodEquipo") or row.get("team_code") or "").strip()
        crest_url = str(
            row.get("url_img") or row.get("url_escudo") or row.get("escudo") or row.get("crest_url") or ""
        ).strip()
        crest_url = sanitize_universo_external_image(absolute_universo_url(crest_url)) if crest_url else ""
        gf = safe_int(
            row.get("gf") or row.get("goles_favor") or row.get("goals_for") or row.get("favor") or row.get("golesFavor")
        )
        ga = safe_int(
            row.get("gc")
