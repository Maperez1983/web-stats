...
def search_web_research(
    query, *, timeout=DEFAULT_TIMEOUT, max_results=MAX_URLS, preferred_domains=None, blocked_domains=None
):
        ...
        return [
            {
                "url": search_url,
                "ok": False,
                "error": f"search_error:{str(exc)[:120]}",
                "title": "",
                "snippet": "",
                "method": "search",
            }
    ...
    if not rows:
        rows.append(
            {"url": search_url, "ok": False, "error": "no_results", "title": "", "snippet": "", "method": "search"}
        )
    else:
        if preferred:
            rows.sort(
                key=lambda row: (
                    0 if _domain_matches(row.get("url") or "", preferred) else 1,
...
def fetch_web_research(raw_urls, *, timeout=DEFAULT_TIMEOUT, max_urls=MAX_URLS):
    ...
    for url in parse_research_urls(raw_urls, limit=max_urls):
        ...
        try:
            ...
            rows.append(
                {"url": clean_url, "ok": False, "error": f"fetch_error:{str(exc)[:120]}", "title": "", "text": ""}
            )
            continue
            ...
            rows.append(
                {
                    "url": url,
                    "ok": False,
                    "error": f"playwright_unavailable:{str(exc)[:100]}",
                    "method": "browser",
                    "title": "",
                    "text": "",
                }
                ...
                for url in valid_urls:
                    ...
                    try:
                        page.goto(
                            ...
                            timeout=max(3000, int(timeout or DEFAULT_TIMEOUT) * 1000),
                        )
                        try:
                        ...
                        rows.append(
                            {
                                "url": url,
                                "ok": False,
                                "error": f"browser_error:{str(exc)[:120]}",
                                "method": "browser",
                                "title": "",
                                "text": "",
                            }
                ...
                rows.append(
                    {
                        "url": url,
                        "ok": False,
                        "error": f"browser_start_error:{str(exc)[:120]}",
                        "method": "browser",
                        "title": "",
                        "text": "",
                    }
