...
def fetch_universo_team_roster(team_code: str) -> list[dict]:
    ...
    for row in squad:
        ...
        if uniq in seen_keys:
            continue
        seen_keys.add(uniq)
        dorsal = _parse_int_safe(
            row.get("dorsal") or row.get("dorsal_jugador") or row.get("numero") or row.get("number") or 0
        )
        roster.append(
            {
                "name": name,
    ...
    stats_enabled = str(os.getenv("UNIVERSO_ROSTER_STATS_ENABLED", "1") or "").strip().lower() in {
        "1",
        "true",
        "yes",
        "on",
