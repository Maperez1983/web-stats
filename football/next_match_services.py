...
def resolve_rival_identity(rival_name, preferred_opponent=None):
    ...
    known_team = (
        Team.objects.filter(
            Q(name__iexact=rival_name) | Q(short_name__iexact=rival_name) | Q(external_id__iexact=rival_name)
        )
    ...
    if isinstance(preferred_opponent, dict):
        ...
        if (
            preferred_key
            and rival_key
            and (preferred_key == rival_key or preferred_key in rival_key or rival_key in preferred_key)
        ):
...
def find_universo_next_match_for_context(context, primary_team):
    if (
        not context
        or str(getattr(context, "provider", "") or "").strip() != WorkspaceCompetitionContext.PROVIDER_UNIVERSO
    ):
        return {}
    ...
    for round_id in rounds:
        ...
        payload = current_payload if round_id == current_round else fetch_universo_live_results(group_key, round_id)
        if not payload:
            continue
...
def _payload_from_universo_result_row(row, *, team_keys, fallback_date="", fallback_round=""):
    ...
    home_code = (
        str(_first(row, "CodEquipo_local", "cod_equipo_local", "CodEquipoLocal", "code_local") or "").strip().lower()
    )
    away_code = (
        str(_first(row, "CodEquipo_visitante", "cod_equipo_visitante", "CodEquipoVisitante", "code_away") or "")
        .strip()
        .lower()
    )
    if home_code:
    ...
    if team_keys & home_keys:
        ...
        home_flag = False
        crest_url = absolute_universo_url(_first(row, "url_img_local", "url_img_loc", "escudo_local", "crest_home"))
...
def load_preferred_next_match_payload(
    ...
    *,
    ...
    competition_context = competition_context or (
        WorkspaceCompetitionContext.objects.filter(Q(team=primary_team) | Q(workspace__primary_team=primary_team))
    ...
    provider_key = (
        str(getattr(competition_context, "provider", "") or "").strip().lower() if competition_context else ""
    )
