...
from . import pdf_services, session_access_services, task_choices, workspace_context
...
from .session_import_services import extract_preview_image_from_pdf
...
SESSION_PDF_DELEGATED_VIEW_NAMES = ("_build_session_pdf_context",)
...
def _is_imported_task(task):
    ...
    if source in {"manual-studio", "manual", "studio"}:
        if (
            bool(getattr(task, "task_pdf", None))
            or str(meta.get("pdf_source_name") or "").strip()
            or bool(meta.get("import_mode"))
        ):
            return True
        return False
...
def _normalize_task_pdf_meta(meta):
    ...
    def _normalize_meta_dict(raw_value):
        ...
        for key, value in raw_value.items():
            ...
            if isinstance(value, dict):
                ...
                normalized_value = [_normalize_meta_dict(item) if isinstance(item, dict) else item for item in value]
            else:
    ...
    if meta.get("coordination_skills"):
        meta["coordination_skills"] = _map_choice(meta.get("coordination_skills"), coord_skills_map) or str(
            meta.get("coordination_skills")
        )
    if meta.get("tactical_intent"):
        meta["tactical_intent"] = _map_choice(meta.get("tactical_intent"), tactical_intent_map) or str(
            meta.get("tactical_intent")
        )
    ...
    if isinstance(meta.get("category_tags"), str):
        meta["category_tags"] = [
            item.strip() for item in str(meta.get("category_tags") or "").split(",") if item.strip()
        ]
    ...
    if isinstance(meta.get("assigned_player_names"), str):
        meta["assigned_player_names"] = [
            item.strip() for item in str(meta.get("assigned_player_names") or "").split(",") if item.strip()
        ]
...
def _build_session_task_sheet(task):
    ...
    return {
        ...
        or "-",
        "players_label": ", ".join(meta.get("assigned_player_names") or [])
        or str(meta.get("player_count") or "").strip()
        or "-",
        ...
        "coordination_skills_label": str(
            meta.get("coordination_skills") or meta.get("coordination_skills_label") or ""
        ).strip(),
...
def build_session_pdf_context(request, team, session, pdf_style="uefa"):
    ...
    if not team_logo_url and _is_benagalbon_team(team):
        try:
            crest_path = (
                ...
                / "football"
                / "images"
                / "cdb-benagalbon-crest-pdf.png"
            )
            raw = crest_path.read_bytes()
            if raw:
                team_logo_url = _small_png_data_url(raw, max_side=220) or (
    ...
    if not team_logo_url:
        try:
            team_logo_url = team_fallback_crest_data_uri(
                team, fallback_label=getattr(team, "display_name", "") or getattr(team, "name", "")
            )
        except Exception:
            team_logo_url = ""
    ...
    uefa_badge_url = _static_data_url("football/images/uefa-badge.svg", "image/svg+xml") or request.build_absolute_uri(
        static("football/images/uefa-badge.svg")
    )
    brand_mark_data_url = _static_data_url("football/images/2j-mark.svg", "image/svg+xml")
    club_dragon_data_url = (
    ...
    def _infer_task_block_for_pdf(task, meta: dict) -> str:
        ...
        if not inferred and (
            "abp" in phase_tags
            or _has_any("abp", "balonparado", "balónparado", "corner", "saquedeesquina", "faltalateral")
        ):
    ...
    def _task_preview_data_url_for_pdf(task, variant: str = "2d"):
        pitch_orientation_value = str((meta or {}).get("pitch_orientation") or "landscape").strip().lower()
        default_canvas_width = 684 if pitch_orientation_value == "portrait" else 1054
        default_canvas_height = 1054 if pitch_orientation_value == "portrait" else 684
        ...
        def _render_perspective_preview_data_url(canvas_state, width=1280, height=720):
            return render_stadium_native_preview_data_url(
                canvas_state if isinstance(canvas_state, dict) else {},
                canvas_width=width or default_canvas_width,
                canvas_height=height or default_canvas_height,
                pitch_orientation=pitch_orientation_value,
            )

        def _render_explicit_2d_preview_data_url(canvas_state, width=1280, height=720):
            ...
            normalized_orientation = pitch_orientation_value
            if raw_grass_style in {"stadium_top", "stadium_top_h", "stadium_top_v", "stadium_native"}:
                return render_stadium_taskboard_preview_data_url(
                    safe_state,
                    canvas_width=max(320, min(_parse_int(width) or default_canvas_width, 3840)),
                    canvas_height=max(180, min(_parse_int(height) or default_canvas_height, 2160)),
        ...
        # Si la tarea usa una superficie premium de estadio, la Vista 2D debe seguir
        # saliendo desde su tablero 2D específico, no desde el render plano legacy.
        raw_grass_style = str(meta.get("pitch_grass_style") or "broadcast_premium").strip().lower()
        allow_saved_preview_for_2d = raw_grass_style not in {
            "stadium_top",
            "stadium_top_h",
            "stadium_top_v",
            "stadium_native",
        }
        if preview and normalized_variant == "2d" and allow_saved_preview_for_2d:
            return _autocrop_preview_data_url(preview)
        if normalized_variant == "3d":
            perspective_preview = _render_perspective_preview_data_url(
                canvas_state, canvas_width or default_canvas_width, canvas_height or default_canvas_height
            )
            if perspective_preview:
                return _autocrop_preview_data_url(perspective_preview)
        if normalized_variant == "2d":
            explicit_2d = _render_explicit_2d_preview_data_url(
                canvas_state, canvas_width or default_canvas_width, canvas_height or default_canvas_height
            )
            if explicit_2d:
                return _autocrop_preview_data_url(explicit_2d)
        ...
        if canvas_state and isinstance(canvas_state, dict) and canvas_state.get("objects"):
            try:
                ...
                elif raw_grass_style in {"stadium_top", "stadium_top_h", "stadium_top_v", "stadium_native"}:
                    pitch_grass_style = "stadium_native"
                elif raw_grass_style in {
                    "classic",
                    "broadcast",
                ...
                pitch_zoom = 1.0
                canvas_width = max(320, min(_parse_int(canvas_width) or default_canvas_width, 3840))
                canvas_height = max(180, min(_parse_int(canvas_height) or default_canvas_height, 2160))
                ...
                if png_bytes:
                    return _autocrop_preview_data_url(
    ...
    def _frame_preview_data_url_for_pdf(task, frame_state, variant: str = "2d"):
        if (
            not isinstance(frame_state, dict)
            or not isinstance(frame_state.get("objects"), list)
            or not frame_state.get("objects")
        ):
            return ""
        layout = task.tactical_layout if isinstance(task.tactical_layout, dict) else {}
        if isinstance(layout, str):
        ...
        normalized_variant = "3d" if str(variant or "").strip().lower() == "3d" else "2d"
        if normalized_variant == "3d":
            perspective_preview = render_stadium_native_preview_data_url(
                frame_state,
        ...
        try:
            png_bytes = render_task_preview_png(
                ...
                pitch_grass_style=pitch_grass_style,
                pitch_zoom=1.0,
                world_width=default_canvas_width,
                world_height=default_canvas_height,
                max_side=2200,
            )
        except Exception:
            png_bytes = None
        if not png_bytes:
            return _task_preview_data_url_for_pdf(task, normalized_variant)
        return "data:image/png;base64," + base64.b64encode(png_bytes).decode("ascii")

    task_cards = []
    for idx, task in enumerate(tasks):
        sheet = task_sheets[idx] if idx < len(task_sheets) else _build_session_task_sheet(task)
        preview_2d_url = _safe_pdf_image_data_url(_task_preview_data_url_for_pdf(task, "2d"))
        preview_3d_url = _safe_pdf_image_data_url(_task_preview_data_url_for_pdf(task, "3d"))
        meta = {}
        try:
        ...
        try:
            ...
            timeline = []
        for frame in timeline[:3]:
            if not isinstance(frame, dict):
                continue
            preview_2d = _frame_preview_data_url_for_pdf(task, frame.get("canvas_state"), "2d")
            preview_3d = _frame_preview_data_url_for_pdf(task, frame.get("canvas_state"), "3d")
            if preview_2d or preview_3d:
                reproduction_frames.append(
                    {
                        "title": str(frame.get("title") or "").strip(),
                        "preview_url": _safe_pdf_image_data_url(preview_2d or preview_3d),
                        "preview_2d_url": _safe_pdf_image_data_url(preview_2d),
                        "preview_3d_url": _safe_pdf_image_data_url(preview_3d),
                    }
                )
        if not reproduction_frames and (preview_2d_url or preview_3d_url):
            reproduction_frames.append(
                {
                    "title": "Situación base",
                    "preview_url": preview_2d_url or preview_3d_url,
        ...
        task_cards.append(
            {
                ...
                "preview_2d_url": preview_2d_url,
                "preview_3d_url": preview_3d_url,
                "reproduction_frames": reproduction_frames,
    ...
    for spec in section_specs:
        cards = [
            ...
            if str(card.get("effective_block") or getattr(card["task"], "block", "") or "") in set(spec["blocks"])
        ]
        cards.sort(
            key=lambda c: (int(getattr(c.get("task"), "order", 0) or 0), int(getattr(c.get("task"), "id", 0) or 0))
        )
        if cards:
    ...
    other_cards = [
        card
        for card in task_cards
        if str(card.get("effective_block") or getattr(card["task"], "block", "") or "") not in known_blocks
    ]
    other_cards.sort(
        key=lambda c: (int(getattr(c.get("task"), "order", 0) or 0), int(getattr(c.get("task"), "id", 0) or 0))
    )
        ...
        sorted(
            {
                ...
                if str(sheet.get("materials_label") or "").strip()
                and str(sheet.get("materials_label") or "").strip() != "-"
            }
        )
    )
