...
def infer_club_season_for_date(workspace, value):
    ...
    if not isinstance(value, date):
        return None
    return (
        WorkspaceSeason.objects.filter(workspace=workspace, start_date__lte=value)
...
def team_season_membership_map(season, teams):
    ...
    if not team_ids:
        return {}
    rows = WorkspaceSeasonTeam.objects.filter(season=season, team_id__in=team_ids).select_related("team")
...
def ensure_player_season_membership(season, player, *, team=None, confirmed=False, status=None):
    ...
    if workspace:
        ensure_workspace_player(
            workspace,
            player,
            current_team=team or getattr(player, "team", None),
...
def mark_player_left_current_season(season, player, *, notes=""):
    ...
    membership = ensure_player_season_membership(
        season, player, team=getattr(player, "team", None), confirmed=False, status=WorkspaceSeasonPlayer.STATUS_LEFT
    )
    if membership:
...
def team_season_history(team):
    ...
    return (
        WorkspaceSeasonTeam.objects.filter(team=team)
...
def player_season_history(player):
    ...
    return (
        WorkspaceSeasonPlayer.objects.filter(player=player)
...
def season_architecture_audit(workspace):
    ...
    by_season = {}
    task_rows = (
        SessionTask.objects.filter(session__microcycle__team_id__in=team_ids)
...
def _season_backfill_specs(team_ids):
    ...
    return [
        (
            "matches",
            Match.objects.filter(home_team_id__in=team_ids) | Match.objects.filter(away_team_id__in=team_ids),
            "date",
        ),
...
def backfill_workspace_club_seasons(workspace, *, dry_run=True):
    ...
    tasks_qs = SessionTask.objects.filter(
        session__microcycle__team_id__in=team_ids, club_season__isnull=True, session__club_season__isnull=False
    ).select_related("session")
    ...
    remaining_qs = SessionTask.objects.filter(
        session__microcycle__team_id__in=team_ids, club_season__isnull=True
    ).select_related("session")
